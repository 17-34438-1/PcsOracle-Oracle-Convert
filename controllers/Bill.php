	...
	function containerBillList()
	{
		...
		{	
			...
			// {
				...
				// $cond = " WHERE agent_code='$agentCode'";
			// }
			...
			// LEFT JOIN ctmsmis.billingdisputecont ON ctmsmis.mis_billing.draft_id= ctmsmis.billingdisputecont.invoicerefno ".$cond."		
			...
			// bill list - intakhab - 2022-09-13 - start
			
			$cond = "";
			if($orgTypeId==1)
			{
				$orgId = $this->session->userdata('org_id');
				$sql_mloList = "SELECT mlo_code FROM organization_logo WHERE org_id='$orgId'";
				$rslt_mloList = $this->bm->dataSelectDB1($sql_mloList);
				
				$mloCode = "";
				
				for($i=0;$i<count($rslt_mloList);$i++)
				{
					$mloCode = $mloCode."'".$rslt_mloList[$i]['mlo_code']."',";
				}
				$mloCode = substr($mloCode,0,-1);
				$cond = " WHERE mlo_code IN($mloCode)";
			}
			...
			".$cond."
			ORDER BY mis_billing.draft_id DESC limit $start,$limit";
			// bill list - intakhab - 2022-09-13 - end
