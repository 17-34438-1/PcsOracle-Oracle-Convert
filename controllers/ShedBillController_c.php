...
class ShedBillController extends CI_Controller {
	...
	function addVryInfoFCL($rotNo,$contNo,$unit_gkey)
	{
		$login_id = $this->session->userdata('login_id');
		$org_license = $this->session->userdata('org_license');
		...
		if($cnt==0)
		{
			...
			$sql_insertQtyTruck = "INSERT INTO verify_info_fcl(igm_detail_id,igm_detail_cont_id,cnf_lic_no,cnf_mobile_no,unit_gkey,rotation,cont_number,no_of_truck,truck_no_by,truck_no_time)
			VALUES('$igmDtlId','$igmDtlContId','$org_license','$smsNo','$unit_gkey','$rotNo','$contNo','$truck_qty','$login_id',NOW())";
   ...
   //P Shed Tally Report Starts------
	...
	function gateInStsCng()
	{
        ...
        else
        {
			...
			$getInQuery = "UPDATE do_truck_details_entry SET gate_in_status = '1' , gate_in_by = '$login_id' ,gate_in_time = NOW() WHERE id='$id'";
	...
	function paymentStsCng()
	{
        ...
        else
        {
			...
			$stsUpdateQuery = "UPDATE do_truck_details_entry SET gate_no='$gate_no',paid_status='1' , paid_collect_dt = NOW() , paid_collect_by = '$login_id' , collect_gate_no = '$gate_no' WHERE id='$id'";
	...
	function chalanGenerationForm()
	{
        ...
        else
        {
			...
			$data['title']="Chalan Generation Form";
			$data['msg'] = "";
			$data['flag'] = 0;
	...
	function cnfTruckEntryForm($rotNo=null,$contNo=null,$cont_status=null,$assignmentType=null,$msg=null)
	{
		...
		{									
			...
			if($this->input->post('editId'))
			{
				...
				(SELECT mobile_number FROM vcms_vehicle_agent WHERE card_number=driver_gate_pass) AS driver_mobile_number,
				assistant_name,assistant_gate_pass,
				(SELECT mobile_number FROM vcms_vehicle_agent WHERE card_number=assistant_gate_pass) AS helper_mobile_number
				FROM do_truck_details_entry
				WHERE id='$editId'";
				$rslt_trkEditInfo = $this->bm->dataSelectDB1($sql_trkEditInfo);
				$data['rslt_trkEditInfo']=$rslt_trkEditInfo;				
	...
	{	
		...
		$result=$this->db->query("select * from users where login_id='$login_id'");
			...
			if($cntApplication==0)
			{
				...
				if($type_of_igm!="")
				{
					...
					{	
						...
						$strInsert = "INSERT INTO edo_application_by_cf(rotation,bl,bl_type,igm_type,sh_agent_org_id,entry_time,sumitted_by,ip_address)
					...
					else
					{
						...
						if(count($rsltStr)>0)
						{
							...
							$strInsert = "INSERT INTO edo_application_by_cf( rotation, bl, bl_type, igm_type, mlo, ff_org_id,ff_stat,ff_clearance_time,cont_status,mbl_of_hbl, entry_time, sumitted_by,ip_address) 
							VALUES('$rot_no','$bl_no','$bl_type','$type_of_igm','$master_org','$sup_org','$ff_stat','$ff_clearance_time','$cont_status','$master_bl' ,NOW(), '$login_id','$ip_address')";
						}
						else
						{
							...
							$strInsert = "INSERT INTO edo_application_by_cf( rotation, bl, bl_type, igm_type, mlo,  entry_time, sumitted_by,ip_address) 
							VALUES('$rot_no','$bl_no','$bl_type','$type_of_igm','$master_org', NOW(), '$login_id','$ip_address')";
						}					
					}
	...
	function tokenDistributionList()
	{
        ...
        else
        {
			...
			if($org_Type_id=="73")
			{
				$queryTokenList="SELECT * FROM token_distribution";
	...
	{		
		...
		if($this->input->post('editFormId'))
		{
			...
			if($editType == "Replace")
			{
				$sql_replaceInfo = "SELECT id,truck_id,driver_name,driver_gate_pass,assistant_name,assistant_gate_pass,paid_amt,paid_method,paid_collect_dt,gate_in_status
				FROM do_truck_details_entry
				WHERE id='$editFormId'";
				$rslt_replaceInfo = $this->bm->dataSelectDB1($sql_replaceInfo);
				...
				$sql_insertReplace = "INSERT INTO vcms_replace_truck_log(visit_id,truck_id,driver_name,driver_gate_pass,assistant_name,assistant_gate_pass,paid_amt,paid_method,paid_collect_dt,replace_time,replace_by,gate_in_status)
				VALUES('$repVisitId','$repTruckId','$repDriverName','$repDriverGatePass','$repAssistantName','$repAssistantGatePass','$repPaidAmt','$repPaidMethod','$repPaidCollectDt',NOW(),'$login_id','$gate_in_status')";
				$this->bm->dataInsertDB1($sql_insertReplace);
	...
	function replaceTruckGateOutForm()
	{
		...
		{
			$data['msg'] = "";
			$data['title'] = "Replace Truck Gate Out Process";
			$data['flag'] = 0;
