...
class VesselBill extends CI_Controller {
	...
	function generateVesselsBillN4()
	{
		...
		else
		{
			...
			{										
				...
				// $chkPangoanVsl = strtoupper(substr($rotation,6,1));
	...
	function vslBillRegenerate()
	{
		...
		else
		{
			...
			$this->vslBillDelete($draftForDelete);
			...
			$sql_updateDispute = "UPDATE ".$this->Init_Table_Map("DETAILS")."
								...
								WHERE ".$this->Init_Table_Map("DETAILS").".rotation='$rotation' 
			...
			$action = "a";
			$msg = "<font color='green'>Bill regenerated for ".$rotation."</font>"
