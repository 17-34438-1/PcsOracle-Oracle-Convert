...
class CI_auth extends CI_Model {
   ...
   public function __construct()
	...
	public function record_count_supplimentary() {
        return $this->db->count_all("igm_supplimentary_detail");
    }
			...
			else
			{
				...
				if($result->num_rows()>0 && $userPass1==$password)
				{
					...
					if($resultP->num_rows()==1)
					{
							...
							write_file($_SERVER['DOCUMENT_ROOT'].'/assets/LogCli/PcsOracle/serverlogin.txt', $data, 'a');
							...
							write_file($_SERVER['DOCUMENT_ROOT'].'/assets/LogCli/PcsOracle/LoginClientSession.log', $data2, 'a');
						...
						//	Break Bulk 26/2/23
                            ...
                            //Break Bulk ship Handling OP 04/06/2023
							if($mdata==105){
								$this->session->set_userdata(array('login_index_id' => $row->id,'login_id'=>$row->login_id,'User_Name'=> $row->u_name,'Control_Panel'=>105,'section'=>$row->section,'n4_bizu_gkey'=>$row->n4_bizu_gkey,'LoginStat'=>"yes",'user_role_id'=> $row->user_role_id,'is_admin_user'=>$row->is_admin_user,'org_Type_id'=>$mdata,'org_id'=> $row->org_id,'org_type'=> $mdata_org,
								'first_login_track'=>$row->first_login_track,'isexpired'=>$row->isexpired,
								'org_license'=>$mdata_license,'org_name'=> $mdata_Organization_Name,'value'=> $this->session->userdata('session_id')));
								return true;
							}
...
");
	...
	*/
        ...
        try {
				...
				{	
					...
					$query2 = $this->db2->query("select vsl_vessels.name as vsl_name,ref_bizunit_scoped.id as agent_code,
							...
							vsl_vessel_berthings.atd as atd,ib_vyg 
	...
	public function dataInsertdb4($str) 
	{
		// Oracle(sparcsn4)
		$CI = &get_instance();
		$this->db2 = $CI->load->database('fourth', TRUE);
		$query=$this->db2->query($str);	  
		return $query;
	}
		
	public function dataUpdatedb4($str) 
	{
		// Oracle(sparcsn4)
		$CI = &get_instance();
		$this->db2 = $CI->load->database('fourth', TRUE);
	...
	public function dataDeletedb4($str) 
	{
		// Oracle(sparcsn4)
		$CI = &get_instance();
		$this->db2 = $CI->load->database('fourth', TRUE);
	...
	public function dataSelectdb4($str)
	{
		// Oracle(sparcsn4)
		$CI = &get_instance();
		$this->db2 = $CI->load->database('fourth', TRUE);
	...
	public function dataReturndb4($str) 
	{
		// Oracle(sparcsn4)
		$CI = &get_instance();
		$this->db2 = $CI->load->database('fourth', TRUE);
	...
	function VesselDataDumpingByDT($fromDate,$toDate)
	{
		...
		for($i=0;$i<count($result);$i++)
		{
			...
			FROM srv_event 
			WHERE srv_event.applied_to_gkey='$vvd_gkey' AND srv_event.event_type_gkey='6092173'";
			$numRows=$this->dataReturn($evenTypeQuery);	

			if($numRows>0)
			{
				$evenTypeUpdateQuery="UPDATE ctmsmis.vsl_vssel_info 
				SET event_type_gkey='6092173' 
				where ctmsmis.vsl_vssel_info.vvd_gkey='$vvd_gkey'";	
