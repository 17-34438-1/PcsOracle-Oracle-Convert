...
class BBExportRotationWiseCargoDischargeModel extends CI_Model
{
...
function ExportRotationWiseCargoDischargeSearch($keyword){
   ...
   function ExportRotationWiseCargoDischargeSearchIGMData($keyword){
   ...
   $query=$this->db->query("SELECT im.Vessel_Name AS VESSEL_NAME,
    ...
    function getVesselInfoByRotation($keyword){


    $query=$this->oracledb->query("SELECT  TO_CHAR(ETA ,'YYYY-MM-DD') as ETA, TO_CHAR(ETD ,'YYYY-MM-DD') as ETD, POS_SLOT AS BERTH FROM VSL_VESSEL_VISIT_DETAILS 
