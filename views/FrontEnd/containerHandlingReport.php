<section class="panel">
	...
	<?php
		...
		$cond = "";
        $terminalCondition=" AND terminal='GCB'";
        ...
        From(
        SELECT ctmsmis.ED.equipment,ctmsmis.EAD.block,ctmsmis.EAD.shift, 
        (SELECT terminal FROM ctmsmis.yard_block WHERE ctmsmis.yard_block.block=EAD.block)AS terminal,
        IFNULL(ctmsmis.EAD.start_work_time,'') AS Start_Work_TIME, 
        IFNULL(ctmsmis.EAD.end_work_time,'') AS End_Work_TIME, 
        IFNULL(ctmsmis.EAD.work_out_time,'') AS Work_Out_Time, 
        CASE EAD.work_out_state WHEN '1' THEN IFNULL(TIMEDIFF(ctmsmis.EAD.work_out_time,ctmsmis.EAD.start_work_time),'') 
        ...
        FROM ctmsmis.mis_equip_assign_detail AS EAD 
        INNER JOIN ctmsmis.mis_equip_detail AS ED ON EAD.equip_detail_id=ED.id
        WHERE ".$cond." ORDER BY ctmsmis.ED.equipment,ctmsmis.EAD.block) AS tbl WHERE tbl.terminal='GCB'";
 ...
 //    exit();						
	?>
		...
		<?php
                                ...
                                $sql="SELECT *
                                From(
                                SELECT ctmsmis.ED.equipment as equipment1,ctmsmis.EAD.block as block1,ctmsmis.EAD.shift, 
                                (SELECT terminal FROM ctmsmis.yard_block WHERE ctmsmis.yard_block.block=EAD.block)AS terminal,
                                IFNULL(ctmsmis.EAD.start_work_time,'') AS Start_Work_TIME, 
                                IFNULL(ctmsmis.EAD.end_work_time,'') AS End_Work_TIME, 
                                IFNULL(ctmsmis.EAD.work_out_time,'') AS Work_Out_Time, 
                                ...
                                FROM ctmsmis.mis_equip_assign_detail AS EAD 
                                INNER JOIN ctmsmis.mis_equip_detail AS ED ON EAD.equip_detail_id=ED.id
                                WHERE ".$cond." ORDER BY ctmsmis.ED.equipment,ctmsmis.EAD.block) AS tbl WHERE tbl.terminal='CCT' ";
			 ...
			 <?php
							/*$sql="select * from ( select * from (
                                    ...
                                    FROM ctmsmis.mis_equip_assign_detail AS EAD 
                                    INNER JOIN ctmsmis.mis_equip_detail AS ED ON EAD.equip_detail_id=ED.id
                                    WHERE ".$cond." ORDER BY ctmsmis.ED.equipment,ctmsmis.EAD.block) AS tbl where tbl.terminal='NCT' ";
