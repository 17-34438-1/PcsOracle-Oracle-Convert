				  ...
				  (CASE WHEN   
					...
					right(sparcsn4.ref_equip_type.nominal_height,2)/10,', Size : ',right(sparcsn4.ref_equip_type.nominal_length,2),',ISO Code : ',sparcsn4.ref_equip_type.id) 
					   WHEN   
						   fcyVisit.last_pos_loctype ='VESSEL'    
					   THEN  
						CONCAT('Position : ',IFNULL(CONVERT(fcyVisit.last_pos_name USING utf8),''),',Vessel Name : ',sparcsn4.vsl_vessels.name,',Category : ',inv.category,',Load Time : ',ifnull(fcyVisit.time_load,''),', MLO : ',g.id,',Status : ',inv.freight_kind,', Height : ',
					right(sparcsn4.ref_equip_type.nominal_height,2)/10,', Size : ',right(sparcsn4.ref_equip_type.nominal_length,2),',ISO Code : ',sparcsn4.ref_equip_type.id)
					   ELSE 
				  ...
				  ',Time Move:',ifnull(fcyVisit.time_move,''),', MLO : ',g.id,', Height : ',
					...
					WHERE inv.id ='$cont_id' ORDER BY inv.gkey DESC LIMIT 1";
						...
						<tr>
							...
							<td ALIGN="CENTER"><?php
							if($cat!=""){
								if($row->dsc && $row->block!=NULL && $row->block!='Y6') 
								{						
									echo $row->dsc;
								$block=$row->block;
								}
								else if($row->dsc && $row->block=='Y6') 
								{						
									echo $row->dsc;
								$block="HS6";
								}
