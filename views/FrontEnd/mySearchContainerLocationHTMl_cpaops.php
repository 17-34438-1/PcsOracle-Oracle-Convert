			...
			<tr>
				...
				<?php
					...
					include("dbOracleConnection.php");
					 ...
					 $strCat = "SELECT category FROM inv_unit WHERE id='$cont_id'
				    ...
				    while(($rowCat= oci_fetch_object($resCat)) != false)
					{
					 ...
					 $sql_contDest = "SELECT desti.destination			
					FROM inv_unit inv
					...
					while(($row_contDest = oci_fetch_object($res_contDest)) != false)
					{
						 ...
						 $sql_contDest2="SELECT ctmsmis.offdoc.name AS dest_name FROM ctmsmis.offdoc WHERE id=$contDest";
						 $res_contDest2 = mysqli_query($con_sparcsn4,$sql_contDest2);
					    $row2 = mysqli_fetch_object($res_contDest2);
					    $contDestName =  $row2 ->dest_name;
					}
					...
					{						
						...
						{		
							...
							$sql="SELECT inv.id,fcyVisit.transit_state,fcyVisit.time_in,fcyVisit.last_pos_slot,
 
							(CASE WHEN   
							   fcyVisit.last_pos_loctype ='YARD'   
								  THEN   
									 ( ',Pos:'||CONVERT(fcyVisit.last_pos_slot , 'utf8')|| 
										',Ctgry:' || inv.category || ',Status:' || inv.freight_kind ||
										',Dest:' || NVL(desti.destination,'') ||
										',MLO:' || g.id ||
										',ISO:' || NVL(ref_equip_type.id,'') || 
										',AssnType:' || NVL((SELECT inv_unit.flex_string01 FROM inv_unit WHERE inv_unit.gkey=inv.gkey),'') ||
										',AssnDate:' || NVL(fcyVisit.flex_date01,'') ||  
										',DisTime:' || NVL(fcyVisit.time_in,'') ||
									  ', Height : ' ||
								 ...
								 THEN  
								(
								'Position : ' || NVL(CONVERT(fcyVisit.last_pos_name , 'utf8'),'') || ',Vessel Name : ' || vsl_vessels.name || ',Category : ' || inv.category || ', MLO : ' || g.id || ', Height : ' ||
							  substr(ref_equip_type.nominal_height,-2)/10 || ', Size : ' || substr(ref_equip_type.nominal_length,-2) || ',ISO Code : ' || ref_equip_type.id
							  )
								 ELSE 
							  NVL(('Position : ' || CONVERT(fcyVisit.last_pos_name , 'utf8') || ',Vessel Name : ' || vsl_vessels.name || ',Category : '  ||  inv.category || ',MLO : ' 
							  || g.id || ', Height : ' ||
							  substr(ref_equip_type.nominal_height,-2)/10 || ', Size : ' || substr(ref_equip_type.nominal_length,-2) || ',ISO Code : ' || ref_equip_type.id),'NO CONTAINER FOUND'
							  )  
						...
						{				
							...
							(CASE WHEN   
									  fcyVisit.last_pos_loctype ='YARD'   
								  THEN   
									  ('Position:' || 'Inside Port' || ',DisTime:' || NVL(fcyVisit.time_in,'')
									  )   
								 WHEN   
									 fcyVisit.last_pos_loctype ='VESSEL'    
								 THEN  
							  ...
							  substr(ref_equip_type.nominal_height,-2)/10 || ', Size : ' || substr(ref_equip_type.nominal_length,-2) || ',ISO Code : ' || ref_equip_type.id
								 ...
								 ELSE 
							  NVL(('Position : ' || CONVERT(fcyVisit.last_pos_name , 'utf8') || ',Vessel Name : ' || vsl_vessels.name || ',Category : ' || inv.category || 'MLO : ' || g.id  || ', Height : ' ||
							  substr(ref_equip_type.nominal_height,-2)/10 || ', Size : ' || substr(ref_equip_type.nominal_length,-2) || ',ISO Code : ' || ref_equip_type.id),'NO CONTAINER FOUND'
							  )  
					...
					else if($cat=="EXPRT") 
					{
						...
						$sql="SELECT inv.id, fcyVisit.transit_state,freight_kind,fcyVisit.last_pos_slot,
						...
						(CASE WHEN   
						  ...
						  substr(ref_equip_type.nominal_height,-2)/10 || ', Size : ' || substr(ref_equip_type.nominal_length,-2) || ', ISO Code : ' || ref_equip_type.id)
							 ELSE 
							('PRE ADVISED : ' || NVL(CONVERT(fcyVisit.last_pos_name , 'utf8'),'') ||
								 ',Category : ' || (inv.category || ', ' || 
								   NVL((SELECT ( CASE WHEN sub_type='DE' THEN 'Dray Off'   
					...
					INNER JOIN ref_equip_type ON ref_equipment.eqtyp_gkey=ref_equip_type.gkey 
					inner join inv_goods desti on desti.gkey=inv.goods
					WHERE inv.id ='MSDU1548318' ORDER BY inv.gkey DESC fetch first 1 rows only";
			...
			<tr>
				<td bgcolor="#B5EFF0">
					<table width="100%"  align="center" border="0">			
						...
						<tr>
							<td width="20%">Last Vessel Name</td><td>:</td><td ><b><?php if($cat == ""){echo "";}else{echo $row2->Vessel_Name;}?></b></td>
						</tr>
						<tr>
							<td width="20%">Import Rotation No</td><td>:</td><td ><b><?php if($cat == ""){echo "";}else{echo $row2->Import_Rotation_No; }?></b></td>
						</tr>
						<tr>
							<td width="20%">Voy No</td><td>:</td><td ><b><?php if($cat == ""){echo "";}else{echo $row2->Voy_No; }?></b></td>
						</tr>
						<tr>
							<td width="20%">Vessel Arrival Date(Estimated)</td><td>:</td><td ><b><?php if($cat == ""){echo "";}else{echo  $row2->file_clearence_date; }?></b></td>
						</tr>
						<tr>
							<td width="20%">Port of Shipment</td><td>:</td><td ><b><?php if($cat == ""){echo "";}else{echo  $row2->Port_of_Shipment; }?></b></td>
						</tr>
						<tr>
							<!--td width="20%">Port/Offdock</td><td>:</td><td ><b><?php if($cat == ""){echo "";}else{echo  $row2->offdock; }?></b></td-->
