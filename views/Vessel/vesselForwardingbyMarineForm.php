				...
				<table class="table table-bordered table-hover table-striped mb-none" id="datatable-default">
						...
						<thead>
							...
							?>
								...
								<!--th rowspan="2" class="text-center">OA Date</th-->
						...
						<?php 
							...
							{ 
						?>
							<td><input type="checkbox" name="idchk[]" id="idchk<?php echo $i;?>"  onclick="selectCheck(this);" value="<?php echo $departData[$i]['vvd']; ?>" 
								<?php 
						...
						<?php
							...
							if($vvd_gkey=="" or $vvd_gkey==null)
							{
								...
								FROM igm_masters
								WHERE Import_Rotation_No='$rot' order by id desc limit 1";
							...
							else
							{
								...
								mooring_to_time,tug_name,assit_frm,assit_to,shift_dt,DATE(pilot_on_board) AS sign_shift,aditional_tug+1 AS aditional_tug
								FROM doc_vsl_shift
								WHERE doc_vsl_shift.vvd_gkey='$vvd_gkey'";
								$rslt_show_current_data=$this->bm->dataSelectDb1($sql_show_current_data);
								if(count($rslt_show_current_data)>0)
								{
									$shft_time=$rslt_show_current_data[0]['mooring_frm_time'];
						...
						?>
							...
							<td>
								...
								?>
									...
									<a href="<?php echo site_url('Report/departureReportOfVessel/R/'.str_replace("/","_",$departData[$i]['ib_vyg']).'/'.$departData[$i]['vvd_gkey'])?>" target="_blank" method="POST">
										<span class="btn btn-primary" >View</span>
									</a>
								<?php
									}
