			...
			<div class="col-lg-12">						
				<section class="panel">
					<div class="panel-body">
						<form class="form-horizontal form-bordered" method="POST" action="<?php echo site_url('Vessel/searchNotEnteringVslFromLetter'); ?>"  id="myform" name="myform">
								...
								<tbody>
									...
									<?php } else if($section=='billop') { 
											...
											$str = "SELECT COUNT(*) AS rtnValue 
												...
												WHERE vsl_frwd_letter_no='$lett_no' AND outer_vsl_forward_info.bill_op_user_id='$login_id'
												AND cchaportdb.outer_vsl_forward_info.billop_bill_stat='0'";
											$taskRslt = $this->bm->dataReturnDb1($str);
									?>
											<td align="center" style="height:5%;"><?php echo $taskRslt;?></td>
									...
									<?php if($section=='acc') { ?>	
										...
										<td align="center" style="height:5%;">
											<form action="<?php echo site_url("Vessel/vesselForwardingLetter_NotEntering");?>" method="post" target="_blank">
										...
										<?php if($section=='acc' ) { ?>	
										<td align="center" style="height:5%;">
											<form action="<?php echo site_url("Vessel/vesselForwardingForAcc_NotEntering");?>" method="post">
												<input type="hidden" name="letter_id" id="letter_id" value="<?php echo $letterList[$i]['letter_no'];?>">
												<input type="hidden" name="file_dt" id="file_dt" value="<?php echo $letterList[$i]['file_dt'];?>">
												<input type="hidden" name="file_sub" id="file_sub" value="<?php echo $letterList[$i]['file_sub'];?>">
												<input type="hidden" name="file_no" id="file_no" value="<?php echo $letterList[$i]['file_no'];?>">
										...
										<?php } ?>
										<td align="center" style="height:5%;">
											<form action="<?php echo site_url("Vessel/vesselForwardingLetter_NotEntering/stmt");?>" method="post" target="_blank">
												<input type="hidden" name="letter_id" id="letter_id" value="<?php echo $letterList[$i]['letter_no'];?>">
												<input type="hidden" name="file_dt" id="file_dt" value="<?php echo $letterList[$i]['file_dt'];?>">
												<input type="hidden" name="file_sub" id="file_sub" value="<?php echo $letterList[$i]['file_sub'];?>">
												<input type="hidden" name="file_no" id="file_no" value="<?php echo $letterList[$i]['file_no'];?>">
												<input type="hidden" name="no_vsl" id="no_vsl" value="<?php echo $letterList[$i]['no_vsl'];?>">
												<input type="hidden" name="stmt_month" id="stmt_month" value="<?php echo $letterList[$i]['stmt_month'];?>">
