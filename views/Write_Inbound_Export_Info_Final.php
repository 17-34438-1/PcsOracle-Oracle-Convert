<?php
	...
	for($i=0;$i<count($result);$i++){
		if($result[$i]['category'] == "EXPRT")
		{
			...
			$unit->setAttribute( "transit-state", "LOADED" );
			...
			$Slotpos = trim($result[$i]['stowage_pos']);
			if($Slotpos == ""){
				$position=$xml->createElement("position");
				$position->setAttribute( "loc-type", "VESSEL" );
				$position->setAttribute( "location", $result[$i]['arcar_id'] );
			} else {
				$position=$xml->createElement("position");
				$position->setAttribute( "loc-type", "VESSEL" );
				$position->setAttribute( "location", $result[$i]['arcar_id'] );
	...
	$filename="LOADING_".str_replace("/","_",$rot_no)."_FINAL";
