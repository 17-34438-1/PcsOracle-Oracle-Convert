...
function upload_remarks(contNo,rotNo)
{
	//alert(contNo);

  $('#remarks_form')[0].reset(); // reset form on modals
  	document.getElementById("remarks_cont").value=contNo;
	document.getElementById("remarks_rot").value=rotNo;
  $('#p_upload').modal('show'); 
       ...
       <div class="panel-body table-responsive">
			<table class="table table-bordered table-responsive table-hover table-striped mb-none" id="datatable-default">
					...
					for($i=0;$i<count($rslt_assignmentList);$i++)
					{
						...
						$contstatus="";
						if($status==0){
						$rotation_no=$rslt_assignmentList[$i]['ROT_NO'];
						$container_no=$rslt_assignmentList[$i]['CONT_NO'];
						$cont_size=$rslt_assignmentList[$i]['SIZ'];
						$cont_height=$rslt_assignmentList[$i]['HEIGHT'];
						$container_status=$rslt_assignmentList[$i]['CONT_STATUS'];
					    //$block_Number = $rslt_assignmentList[$i]['BLOCK_NO'];
						$mfdch_vlue=$rslt_assignmentList[$i]['MFDCH_VALUE'];
						$assignment_date=$rslt_assignmentList[$i]['ASSIGNMENTDATE'];
						$unitGkey=$rslt_assignmentList[$i]['UNIT_GKEY'];
						...
						$contstatus=$rslt_assignmentList[$i]['CONT_STATUS'];
							...
							$rotation_no=$rslt_assignmentList[$i]['rot_no'];
							$container_no=$rslt_assignmentList[$i]['cont_no'];
							$cont_size=$rslt_assignmentList[$i]['size'];
							$cont_height=$rslt_assignmentList[$i]['height'];
							$container_status=$rslt_assignmentList[$i]['cont_status'];
							$block_Number=$rslt_assignmentList[$i]['Block_No'];
							$mfdch_vlue=$rslt_assignmentList[$i]['mfdch_value'];
						...
						WHERE Import_Rotation_No='".$rotation_no."' AND cont_number='".$container_no."'";
						...
						else if($contstatus=="LCL")
						{
							...
							WHERE import_rotation='".$rotation_no."' AND cont_number='".$container_no."' 
							LIMIT 1";
					...
					<tr class="gridLight" align="center">
						...
						<td><?php echo $container_status; ?></td>
						<td><?php echo $rotation_no; ?></td>
						<td><?php echo $blNo; ?></td>
						 <td><?php echo $block_Number; ?></td> 
						<td><?php echo $mfdch_vlue; ?></td>
						<td><?php echo $assignment_date; ?></td>
						<!--td>
						...
						$result = $this->bm->chkBlockedContainerforTruckEntry($container_no,$rotation_no,$blNo);
						...
						<td>
							...
							<form name="truckEntryForm" id="truckEntryForm" method="post" action="<?php echo site_url('ShedBillController/cnfTruckEntryForm'); ?>">								
								<input type="hidden" name="rotNo" id="rotNo" value="<?php echo $rotation_no; ?>" />
								<input type="hidden" name="contNo" id="contNo" value="<?php echo $container_no; ?>" />
								<input type="hidden" name="cont_status" id="cont_status" value="<?php echo $container_status; ?>" />
								<input type="hidden" name="unit_gkey" id="unit_gkey" value="<?php echo $unitGkey; ?>" />
								<input type="hidden" name="assignmentType" id="assignmentType" value="<?php echo $mfdch_vlue; ?>" />
						...
						?>
							...
							<!--form name="truckEntryForm" id="truckEntryForm" method="post" action="<?php echo site_url('ShedBillController/bilSearchByVerifyNumber'); ?>"-->								
								<input type="hidden" name="rotNoRemarks" id="rotNoRemarks" value="<?php echo $rotation_no; ?>" />
								<input type="hidden" name="contNoRemarks" id="contNoRemarks" value="<?php echo $container_no; ?>" />
								<input type="hidden" name="cont_status" id="cont_status" value="<?php echo $container_status; ?>" />
								<input style="width:100px" type="submit"  value="Remarks" class="btn btn-xs btn-primary"  
									onclick="upload_remarks('<?php echo $container_no;?>','<?php echo $rotation_no;?>')" />
						</td>
						<?php 
						} 
						if($org_Type_id==62) 
						...
						<td>
							<?php
								...
								WHERE igm_details.Import_Rotation_No='$rotation_no' AND igm_detail_container.cont_number='$container_no'";
						...
						<td>
							<?php 	
								//$custom_remarks=$rslt_assignmentList[$i]['CUSTIM_REMARKS'];
...
<div class="modal fade" id="p_upload"  role="dialog">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h3 class="modal-title" align="center">Remarks</h3>
      </div>
        
      <form action="<?php echo site_url('Report/assignmentCustomRemaksEntry');?>" id="remarks_form" method="POST" class="form-horizontal" enctype="multipart/form-data">    
      <div class="modal-body form">
          <input type="hidden" name="remarks_rot" id="remarks_rot" />
          <input type="hidden" name="remarks_cont" id="remarks_cont" />
          <div class="form-body">
            <div class="form-group">
              <label class="control-label col-md-3">Remarks: </label>
              <div class="col-md-9">
                  <input name="custom_remarks" id="custom_remarks"  class="form-control" type="text" > 
              </div>
            </div>          
          </div>
          ...
          <div class="modal-footer">
            <input type="submit" value="Submit" id="btnSave" name="btnSave" class="btn btn-primary  btn-xs">
           </form>  
