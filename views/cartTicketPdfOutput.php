...
{ 
	...
	WHERE shed_tally_info.verify_number='$verifyNo' LIMIT 1
	UNION ALL
	...
	{
		$sqlCartTicket="SELECT igm_details.id,IFNULL(SUM(rcv_pack+loc_first),0) AS rcv_pack,igm_masters.Vessel_Name,igm_details.Import_Rotation_No,igm_detail_container.cont_number,Pack_Marks_Number,shed_loc,shed_yard,LEFT(Description_of_Goods,30) AS Description_of_Goods,Notify_name,IFNULL(shed_tally_info.verify_number,0) AS verify_number,IFNULL(shed_tally_info.id,0) AS verify_id,igm_details.Pack_Number,igm_details.Pack_Description,igm_details.BL_No,igm_detail_container.cont_size,igm_detail_container.cont_status,igm_detail_container.cont_weight,verify_other_data.cnf_name,verify_other_data.be_no,verify_other_data.be_date,igm_detail_container.cont_height,truck_id,gate_no,cus_rel_odr_no
		FROM  igm_details
		INNER JOIN igm_detail_container ON igm_detail_container.igm_detail_id=igm_details.id
