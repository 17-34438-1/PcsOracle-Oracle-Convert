            ...
            <?php
				if($flag == 1)
				{
					...
					while($row = mysqli_fetch_object($truckResult))
					{
						...
						$challanConfSt = $row->chalan_conf_st;
					...
					// intakhab
					$sql_blNo = "SELECT lcl_dlv_assignment.bl_no
					...
					$blNo = "";
					while($row_bl = mysqli_fetch_object($rslt_blNo))
					{
						$blNo = $row_bl->bl_no;
					}
					
					$sql_descGoods = "SELECT igm_supplimentary_detail.Description_of_Goods
					FROM igm_supplimentary_detail
					WHERE igm_supplimentary_detail.Import_Rotation_No='$rot_no' AND igm_supplimentary_detail.BL_No='$blNo'";
					$rslt_descGoods = mysqli_query($con_cchaportdb,$sql_descGoods);					
					
					if(count($rslt_descGoods)==0)
					{
						$sql_descGoods = "SELECT igm_details.Description_of_Goods
						FROM igm_details
						WHERE igm_details.Import_Rotation_No='$rot_no' AND igm_details.BL_No='$blNo'";
						$rslt_descGoods = mysqli_query($con_cchaportdb,$sql_descGoods);	
                        $description_of_Goods="";
						while($row_descGoods = mysqli_fetch_object($rslt_descGoods))
						{
							$description_of_Goods = $row_descGoods->Description_of_Goods;
						}
					...
					if($id != ""){
						...
						if(!is_null($verifyOtherDataId))
						{
							$jettySarkarQuery = "SELECT DISTINCT agent_name,agent_code,lcl_dlv_assignment.cnf_lic_no
						...
						else if(!is_null($verify_info_fcl_id))
						{
						...
						// intakhab - commented on 2022-06-14
						// $cnfLicQuery = "SELECT organization_profiles.License_No
						// FROM do_truck_details_entry 
						// INNER JOIN organization_profiles ON CONCAT(organization_profiles.AIN_No_New,'CF')=do_truck_details_entry.update_by
						// WHERE do_truck_details_entry.id='$id'";

						// $rsltCnf = mysqli_query($con_cchaportdb,$cnfLicQuery);
	
						// while($cnfInfo = mysqli_fetch_object($rsltCnf)){
							// $cnf_lic_no = $cnfInfo->License_No;
						// }
						
						if($cnf_lic_no == "")		// intakhab - 2022-06-14
						{
							$cnfLicQuery = "SELECT organization_profiles.License_No
							FROM do_truck_details_entry 
							INNER JOIN organization_profiles ON CONCAT(organization_profiles.AIN_No_New,'CF')=do_truck_details_entry.update_by
						 ...
						 $rsltCnf = mysqli_query($con_cchaportdb,$cnfLicQuery);
							...
							if(count($rsltCnf)==0)
							{
								$cnfLicQuery = "SELECT organization_profiles.License_No
								FROM do_truck_details_entry 
								INNER JOIN organization_profiles ON CONCAT(organization_profiles.AIN_No,'CF')=do_truck_details_entry.update_by
								WHERE do_truck_details_entry.id='$id'";
							}
					...
					WHERE ref_bizunit_scoped.id = '$cnf_lic_no' AND
					(ref_bizunit_scoped.address_line1 IS NOT NULL OR ref_bizunit_scoped.sms_number IS NOT NULL)";
					
					
					$CNFresult = oci_parse($con_sparcsn4_oracle, $CNFStr1);
                    oci_execute($CNFresult);
					...
					while(($rowCNF= oci_fetch_object($CNFresult)) != false)
					{
					 $cnf=$rowCNF->NAME;
					}
					...
					while(($cnfResult= oci_fetch_object($rsltCnf)) != false){
						...
						while($cnfResult2=mysqli_fetch_object($rsltCnf2)){
							$Yard_No = $cnfResult2->Yard_No;
						}
				...
				<div class="invoice">
					<div class="row">
						<div class="col-sm-6 col-md-6">
							...
							<h6 class="h6 mt-none mb-sm">
								<div class="row">
									<div class="col-md-4">9. C&F Name</div>
									<div class="col-md-1">:</div>
									<div class="col-md-7"><b><?php echo $cnf; ?></b></div>
								</div>
							</h6>
						</div>
						<div class="col-sm-6 col-md-6">
					...
					<div class="row">
						<div class="col-sm-12 text-center">
                            ...
                            if($traffic_chk_st == 1 && $cnf_chk_st == 1 && ($yard_security_chk_st == 1 || $orgTypeId == 28))
							{	
							...
							<?php
								...
								{
							?>
								<!-- challan confirm - start -->
								<div class="col-sm-4" >
									<form method="POST" action="<?php echo site_url("ShedBillController/challanConfirmation") ?>">
										<input type="hidden" name="visitId" id="visitId" value="<?php echo $visitId;?>" />
										<input type="hidden"  name="challanSt" id="challanSt" value="<?php echo $challanConfSt;?>" />
										<button type="submit" name="submit" id="submit" value="challanConfirm" class="mb-xs mt-xs mr-xs btn btn-success" 
										<?php if($challanConfSt==1){ ?>disabled<?php } ?>>Challan Confirm</button>
									</form>
								</div>
								<!-- challan confirm - end -->
								<div class="col-sm-4" >
									<form method="POST" action="<?php echo site_url("GateController/chalanVCMS") ?>" target="_blank">
										...
										<button type="submit" name="submit" id="submit" value="single" class="mb-xs mt-xs mr-xs btn btn-primary">Chalan Print</button>
									</form>
								</div>
								<div class="col-sm-4">
									<form method="POST" action="<?php echo site_url("GateController/chalanVCMS") ?>" target="_blank">
										<input type="hidden" class="btn btn-primary" name="cnf_lic_no" id="cnf_lic_no" 
													value="<?php echo $cnf_lic_no;?>"/>
										<input type="hidden" class="btn btn-primary" name="cnfId" id="cnfId" 
													value="<?php echo $updateBy;?>"/>
										<button type="submit" name="submit" value="all" id="submit" class="mb-xs mt-xs mr-xs btn btn-primary">Print All Chalan for this C&F</button>
									</form>
								</div>
							<?php
								}
