...
<?php 
	...
	ORDER BY cir_no ";
	$query_icd_mst=mysqli_query($con_cchaportdb,$get_icd_mst);
	$i=0;
	while($row_icd_mst=mysqli_fetch_object($query_icd_mst))
	{
	$i++;
?>
...
<div <?php if($i<mysql_num_rows($query_icd_mst)) { ?> class="pageBreak" <?php } else { ?> class="pageBreakOff " <?php } ?>>
	...
	$query_icd_dtl=mysqli_query($con_cchaportdb,$get_icd_dtl);
	$j=0;
	$val_20=0;
	$val_40=0;
	$val_tot=0;
	while($row_icd_dtl=mysqli_fetch_object($query_icd_dtl))
	{
	$j++;
	...
	$query_igm_data=mysqli_query($con_cchaportdb,$get_igm_data);
	$row_igm_data=mysqli_fetch_object($query_igm_data);
	?>
	<tr  align="center">
