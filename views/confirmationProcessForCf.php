	...
	function showJsPic(jsGatePass)
	{
		...
		xmlhttp.onreadystatechange=function(){
			...
			{							  
				...
				{
					document.getElementById('jsPass').value = jsGatePass;
					var jsPic = jsonData.result;
					var picLink = "/biometricPhoto/"+jsGatePass+"/"+jsPic;
					document.getElementById('jsPic').src = picLink;
				}
				else{
					document.getElementById('jsPic').src = "";
					document.getElementById('jsPass').value = "";
				}
			}
		};
		
		var url = "<?php echo site_url('AjaxController/showJsPic')?>?jsGatePass="+jsGatePass;
...
<?php
	...
	$truckQuery="SELECT do_truck_details_entry.id,truck_id,gate_no,driver_name,driver_gate_pass,assistant_name,assistant_gate_pass,
				...
				load_st,import_rotation,cont_no,visit_time_slot_start,visit_time_slot_end,paid_method,paid_status,verify_other_data_id,add_truck_st,conf_for_cf_by,do_truck_details_entry.update_by,u_name
	...
	while($truckResult = mysqli_fetch_object($rsltTruck)){
		...
		$conf_for_cf_by = $truckResult->conf_for_cf_by;
		
        $chk_status = $truckResult->cnf_chk_st;
        if($chk_status==1)
        {
            $btnLabel = "Already Confirmed for ".$cnf_chk_by." by ".$conf_for_cf_by." !";
        }
        else
        {
            $btnLabel = "Confirm";
	...
	if(is_null($verify_info_fcl_id))
	{
		...
		// intakhab - 2021-07-08
		// $jettySarkarQuery = "SELECT DISTINCT agent_name,agent_code,card_number,agent_photo 
		...
		(SELECT agent_photo FROM vcms_vehicle_agent WHERE card_number=driver_gate_pass ORDER BY agent_photo DESC LIMIT 1) AS driver_photo  
		FROM vcms_vehicle_agent 
		INNER JOIN lcl_dlv_assignment ON lcl_dlv_assignment.jetty_sirkar_id = vcms_vehicle_agent.id
		INNER JOIN do_truck_details_entry ON do_truck_details_entry.verify_other_data_id = lcl_dlv_assignment.id
		WHERE do_truck_details_entry.verify_other_data_id='$verifyOtherDataId' AND do_truck_details_entry.id='$visit_id'";
	...
	WHERE inv_unit.id=a.id ORDER BY inv_unit_fcy_visit.flex_date01 DESC fetch first 1 rows only
	) AS rot_no,
							...
							<div class="invoice">
								<div class="row">
									...
									<div class="col-sm-6 col-md-6">
                                        ...
                                        <h6 class="h6 mt-none mb-sm" style="background-color:#f0f1f2;">
											<div class="row">
												<div class="col-md-4">5. Jetty Sircar ID</div>
												<div class="col-md-1">:</div>
												<div class="col-md-7"><b><?php echo $jettySircarId; ?></b></div>
											</div>
										</h6>
										<h6 class="h6 mt-none mb-sm" style="background-color:#f0f1f2;">
											<div class="row">
												<div class="col-md-4">6. Jetty Sircar Name</div>
												<div class="col-md-1">:</div>
										...
										<h6 class="h6 mt-none mb-sm" style="background-color:#f0f1f2;">
											<div class="row">
												<div class="col-md-4">7. Jetty Sircar Lic. No.</div>
												<div class="col-md-1">:</div>
...
<?php
										...
										<h6 class="h6 mt-none mb-sm">
											<div class="row text-center">
												<div class="col-md-12 text-center">
													<img src="<?php echo '/biometricPhoto/'.$jettySircarId.'/'.$jettyPhoto; ?>" id="jsPic" height="120px" width="120px">
							...
							<div class="row">
								<div class="col-sm-12 text-center">
									<form method="post" action="<?php echo site_url('ShedBillController/updateChkStatusForCf'); ?>" 
										...
										<input type="hidden" name="jetty_name" value="<?php echo $agentName;?>">
...
<script>
	function validate()
	{
		...
		if(jsName == ""){
			var selectJs = document.getElementById("jsGatePass").value;
			if(selectJs == ""){
				alert("Please Select Jetty Sircar");
				return false;
			}
		}
