	...
	<table width="100%">
	  <thead>
		...
		<tr bgcolor="#ffffff" height="50px">
			<th align="center" colspan="12"><font size="5"><b>MANLESS GATE INWARD & OUTWARD CONTAINER REGISTER</b></font></th>
		</tr>
...
//) AS tmp ORDER BY shift";
		...
		AS line_id,
		(CASE 
		WHEN entry_dt_time between CONCAT('$date',' 08:00:00') AND CONCAT('$date',' 16:00:00') THEN 'Shift A'
		WHEN entry_dt_time between CONCAT('$date',' 16:00:01') AND CONCAT('$date',' 23:59:59') THEN 'Shift B'
		WHEN entry_dt_time between CONCAT(DATE_ADD('$date',INTERVAL 1 DAY),' 00:00:00') AND CONCAT(DATE_ADD('$date',INTERVAL 1 DAY),' 07:59:59') THEN 'Shift C'
		END) AS shift
		FROM ctmsmis.mis_ocr_info
        ...
        //					WHERE ctmsmis.mis_ocr_info.entry_dt='$date'";
				$row_query_reg=mysqli_query($con_sparcsn4,$getRegisterDtl);
//               $rtn_query_reg=mysql_fetch_object($row_query_reg);
        $shift="";
        $tot20 = 0;
        while($rtn_query_reg=mysqli_fetch_object($row_query_reg)){
