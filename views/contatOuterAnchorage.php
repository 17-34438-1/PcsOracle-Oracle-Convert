<?php
    if($option == 'xl')
    {
		...
		header("Content-Disposition: attachment; filename=Outer_Anchorage.xls;");
...
<html>
    ...
    <body>
        <table width="100%" align="center">
            <tr width="100%">
                <th align="center" colspan="38">
                    <img align="center"  width="235px" height="75px" src="<?php echo IMG_PATH?>cpanew.jpg">
                </th>
            </tr>
            <tr width="100%">
                <th align="center" colspan="38">
        ...
        <table border="1" align="center" width="100%" style="border-collapse:collapse;">
            ...
            <tbody>
                <?php
                    ...
                    /*$vsl_query = "SELECT sparcsn4.vsl_vessels.name,sparcsn4.vsl_vessel_visit_details.ib_vyg,sparcsn4.vsl_vessel_visit_details.ob_vyg,
                    SUBSTR(sparcsn4.argo_carrier_visit.phase,3) AS phase_str,sparcsn4.argo_visit_details.eta,
                    ...
                    WHERE '$date' BETWEEN DATE(ata) AND DATE(atd)
					...
					where TO_DATE('$date','YYYY-MM-DD') BETWEEN cast(ata as date) AND cast(atd as date)
				   ...
				   $vsl_query ="SELECT vsl_vessels.name,vsl_vessel_visit_details.ib_vyg,vsl_vessel_visit_details.ob_vyg,
                    ...
                    $rotation = "";
                    $vsl_name = "";
                   // $vsl_rslt=mysqli_query($con_sparcsn4,$vsl_query);
				   $vsl_rslt= oci_parse($con_sparcsn4_oracle,$vsl_query);
                   oci_execute($vsl_rslt);
                    $i=1;
                   // while($vsl=mysqli_fetch_object($vsl_rslt))
                ...
                <tr>
                    ...
                    <?php
                       ...
                       /* $rfr_query = "SELECT SUM(ft_20) AS ft_20, SUM(ft_40) AS ft_40, ((SUM(ft_40)*2) +SUM(ft_20)) AS teus, COUNT(*) AS box  FROM (
                        SELECT inv_unit.id,
                        vsl_vessels.name AS vsl_name,
                        vsl_vessel_visit_details.ib_vyg AS vsl_visit_dtls_ib_vyg,
                        (CASE WHEN RIGHT(sparcsn4.ref_equip_type.nominal_length,2) = 20 THEN 1 ELSE 0 END) AS ft_20,
                        (CASE WHEN RIGHT(sparcsn4.ref_equip_type.nominal_length,2)> 20 THEN 1 ELSE 0 END) AS ft_40,
                        ...
                        INNER JOIN sparcsn4.vsl_vessels ON sparcsn4.vsl_vessels.gkey=sparcsn4.vsl_vessel_visit_details.vessel_gkey
                        WHERE sparcsn4.ref_equip_type.iso_group IN ('RE','RS','RT') 
                        AND vsl_vessel_visit_details.ib_vyg='$rotation') AS tmp";*/
                        ...
                        //$rfr_rslt=mysqli_query($con_sparcsn4,$rfr_query);
						$rfr_rslt = oci_parse($con_sparcsn4_oracle, $rfr_query);
                        oci_execute($rfr_rslt);
						    // echo '<pre>';
		                    // print_r();
		                    // echo '</pre>';
                       // while($rfr_row=mysqli_fetch_object($rfr_rslt))
