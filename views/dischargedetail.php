		...
		<table align="center" width="95%">
			...
			<?php
				...
				for($i=0; $i<count($rslt_detail); $i++) {
					...
					$strQuPreloc="select inv_goods.destination
					from inv_unit 
					inner join inv_goods on inv_goods.gkey=inv_unit.goods
					where inv_unit.gkey='$gkey'";
					$strPrelocRes= oci_parse($con_sparcsn4_oracle, $strQuPreloc);
                    oci_execute($strPrelocRes);
					$row2 = oci_fetch_object($strPrelocRes);
					$preloc="";
					$preloc=$row2 ->DESTINATION;
				...
				// Free the statement identifier when closing the connection
                 oci_free_statement($strPrelocRes);
