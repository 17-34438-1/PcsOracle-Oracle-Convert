 <?php
	...
	//$con = mysqli_connect("192.168.16.42","user1","user1test","cchaportdb");
	
	include('dbOracleConnection.php');
	//$con1 = mysqli_connect("10.1.1.21","sparcsn4","sparcsn4","sparcsn4");
...
$queryStr = "select BL_NO,Notify_name,Line_No,bill_of_entry_date,supDtl.Import_Rotation_No,Vessel_Name,Voy_No 
			...
			where supDtl.Bill_of_Entry_No='$be_no'";
$resQuery=mysqli_query($con_cchaportdb,$queryStr);
$beDate ="";
$notifyName ="";
while($rowequip=mysqli_fetch_object($resQuery))
{
	$blNo= $rowequip->BL_NO;
...
$queryStrSpars = "select to_char( ata, 'yyyy-mm-dd') ata ,name from argo_carrier_visit 
...
$resQuerySprs=oci_parse($con_sparcsn4_oracle,$queryStrSpars);
oci_execute($resQuerySprs,OCI_DEFAULT);
				
//$resQuerySprs=mysqli_query($con1,$queryStrSpars);
//while($rowequipSprs=mysqli_fetch_object($resQuerySprs))
$ata = "";
while (($rowequipSprs = oci_fetch_object($resQuerySprs)) != false)
{
		$vslName= $rowequipSprs->NAME;
...
<div align="left" style="padding-left:10%;">
	...
	Customs B/E No & C.No <u><?php echo $be_no?></u> Date <u><?php echo $beDate?></u> Delivery Order granted against original Bill of Lading.
</div>
...
<div style="padding-left:10%;padding-right:10%;">
	...
	<table border=1px >
		...
		<?php 
			...
			inner join igm_masters im on supDtl.igm_master_id= im.id
			where supDtl.Bill_of_Entry_No='$be_no'";
			$resQuery=mysqli_query($con,$queryStr);
			while($rowequip=mysqli_fetch_object($resQuery))
			{
