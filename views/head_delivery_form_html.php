...
<table align="center" width="850px">
	<tr>
		<td align="center"><img align="middle" width="235px" height="75px" src="<?php echo ASSETS_WEB_PATH?>fimg/cpanew.jpg"></td>
	</tr>
	<tr>
...
<table width="850px" align="center" border="2" style="border-collapse:collapse;">
	<tr>
		...
		<td colspan="8" align="center">C&F : <?php echo $rslt_info_panel[0]['cf']; ?>, Vessel : <?php echo isset($rslt_info_panel[0]['v_name']) ? $rslt_info_panel[0]['v_name'] : ""; ?>, Rotation : <?php echo $rslt_info_panel[0]['rot_no']; ?>, BL No. : <?php echo $rslt_info_panel[0]['bl_no']; ?></td>
	</tr>
	<tr>
		<td align="center"><b>B/E No</b> :</td>
		...
		<td align="center"><b>Exit Number</b> :</td>
		<!--td><input style="width:150px" name="be_no_tmp" id="be_no_tmp" value="<?php echo $rslt_entry_dlv_dtl[0]['be_no']; ?>" <?php if($is_entry_data==1){?> readonly <?php } ?> /></td-->
		<td align="center"><?php echo $exit_note_number; ?></td>
		<td align="center"><b>B/E Date</b> :</td>
		...
		<td align="center"><b>STC</b> :</td>
		<td><?php echo $rslt_stc_weight[0]['stc']; ?></td>
	</tr>
	<tr>
		<td colspan="2" align="center"><b>CP No</b> :</td>
		<td><input style="width:150px" name="cp_no_tmp" id="cp_no_tmp" value="<?php echo isset($rslt_entry_dlv_dtl[0]['cp_no']) ? $rslt_entry_dlv_dtl[0]['cp_no'] : ""; ?>" <?php if(isset($is_entry_data) ? $is_entry_data : 0==1){?> readonly <?php } ?> /></td>
		<td><b>CP Date</b> :</td>
		<td align="center"><input style="width:150px" type="date" name="cp_date_tmp" id="cp_date_tmp" value="<?php echo isset($rslt_entry_dlv_dtl[0]['cp_dt']) ? $rslt_entry_dlv_dtl[0]['cp_dt'] : ""; ?>" <?php if(isset($is_entry_data) ? $is_entry_data : 0==1){?> readonly <?php } ?> /></td>
				<!--script>
					$(function() {
						$( "#cp_dt" ).datepicker({
							changeMonth: true,
		...
		<td><b>Weight</b> :</td>
		<td colspan="2"><?php echo $rslt_stc_weight[0]['weight']; ?></td>
	</tr>
<!--/table>
<br>
<table width="800px" align="center" border="2" style="border-collapse:collapse;"-->
	<tr>
		...
		<th align="center" colspan="2">Good's Description</th>
		<th colspan="2" align="center">Cart Details</th>
		<th align="center">Delivery</th>
		<th align="center" colspan="2">Signature & Mobile</th>
	</tr>
	<?php
	...
	{		
		...
		$rslt_unit_gkey = oci_parse($con_sparcsn4_oracle, $sql_unit_gkey);
        oci_execute($rslt_unit_gkey);
		                  
		$tot_dlv_qty = "";
		// while($row_unit_gkey=mysqli_fetch_object($rslt_unit_gkey))
		
		while(($row_unit_gkey=oci_fetch_object($rslt_unit_gkey))!=false)
		{

			//echo $row_unit_gkey->GKEY;
			$sql_chk_status="SELECT dlv_status,status_dt
			FROM ctmsmis.mis_head_delivery_detail
			WHERE unit_gkey='$row_unit_gkey->GKEY'";
				
			$rslt_chk_status=mysqli_query($con_sparcsn4,$sql_chk_status);
				
			$row_chk_status=mysqli_fetch_object($rslt_chk_status);
			
			$chk_status=isset($row_chk_status->dlv_status) ? $row_chk_status->dlv_status : "";
			$chk_date=isset($row_chk_status->status_dt) ? $row_chk_status->status_dt : "";
	?>
	<tr>
		<td align="center"><?php echo $rslt_cont_no[$i]['cont_number']; ?></td>
		<td align="center" colspan="2"><?php echo $rslt_cont_no[$i]['Description_of_Goods']; ?></td>
		<td colspan="2" align="center">
			...
			<form name="truck_entry_form" id="truck_entry_form" method="post" action="<?php echo site_url("report/head_delivery_entry");?>">
				...
				<table border="1" style="border-collapse:collapse;margin:0 0 0 0;width:300px;" >
					...
					$sql_truck_info = "SELECT id,truck_id, actual_delv_pack FROM cchaportdb.do_truck_details_entry WHERE cont_no='$cont_id'";
					
					// $rslt_truck_qty=mysqli_query($con_sparcsn4,$sql_truck_qty);
					$rslt_truck_qty=mysqli_query($con_cchaportdb,$sql_truck_info);
					
					while($row_truck_qty=mysqli_fetch_object($rslt_truck_qty))
					{
					?>
					<tr>
						<!-- <td><?php echo $row_truck_qty->truck_no; ?></td>
						<td><?php echo $row_truck_qty->qty; ?></td> -->
						<td><input type="text" id="truck_id" name="truck_id" value="<?php echo $row_truck_qty->truck_id; ?>"></td>
						<td><input type="text" id="qty" name="qty" value="<?php echo $row_truck_qty->actual_delv_pack; ?>"></td>
						<td align="center">
		...
		<td align="center" colspan="2">
			...
			$rslt_cf_sign=mysqli_query($con_sparcsn4,$sql_cf_sign);
			
			$row_cf_sign=mysqli_fetch_object($rslt_cf_sign);
			
			$cf_sign=isset($row_cf_sign->signature_path) ? $row_cf_sign->signature_path : "";
			...
			src="<?php echo IMG_RESOURCE_PATH; ?><?php echo "jetty_sarkar_signature_files/".$cf_sign; ?>" />
		</td>
	</tr>
	<?php
		}
	...
	oci_free_statement($rslt_unit_gkey);
