...
<tr bgcolor="#ffffff" align="center" height="100px">
		<td colspan="13" align="center">
			<table border=0 width="100%">
				...
				<tr align="center">
					<td><img align="middle"  width="235px" height="80x" src="<?php echo ASSETS_WEB_PATH?>fimg/cpanew.jpg"></td></tr>
...
<?php
	...
	$mlo_query="SELECT  DISTINCT (sparcsn4.ref_bizunit_scoped.id), Y.id AS agent
...
ORDER BY ref_bizunit_scoped.id ASC";
	$mlo_query=mysqli_query($con_sparcsn4,$mlo_query);
	$mlo=0;	
	while($mlo_row=mysqli_fetch_object($mlo_query)){
	$mlo++;
...
<?php
	...
	$query="SELECT sparcsn4.inv_unit.gkey, sparcsn4.inv_unit.id as cont, sparcsn4.ref_bizunit_scoped.id as mlo, inv_unit_fcy_visit.time_in,
	inv_unit_fcy_visit.time_out, inv_goods.destination,category,
(SELECT placed_time FROM sparcsn4.srv_event WHERE applied_to_gkey=sparcsn4.inv_unit.gkey AND event_type_gkey=20) AS ou,	freight_kind,
...
<tr align="center">
		...
		<td><?php if($row->ou) echo $row->ou; else echo "&nbsp;";?></td>
</tr>
...
WHERE sparcsn4.vsl_vessel_visit_details.ib_vyg='$ddl_imp_rot_no' AND ref_bizunit_scoped.id ='$mlo_row->id' AND inv_goods.destination=2592 AND inv_unit_equip.eq_role='PRIMARY') AS tmp";
	$summary_query=mysqli_query($con_sparcsn4,$summary_query);
	$sum=0;	
	while($summary_row=mysqli_fetch_object($summary_query)){
	$sum++;
...
<?php
	...
	$mlo_query_2="SELECT DISTINCT(r.id), Y.id AS agent, Y.name AS agent_name
FROM sparcsn4.inv_unit 
INNER JOIN sparcsn4.inv_unit_equip ON sparcsn4.inv_unit_equip.unit_gkey=inv_unit.gkey
INNER JOIN sparcsn4.ref_equipment ON sparcsn4.ref_equipment.gkey=sparcsn4.inv_unit_equip.eq_gkey
INNER JOIN sparcsn4.ref_equip_type ON sparcsn4.ref_equip_type.gkey=sparcsn4.ref_equipment.eqtyp_gkey
...
INNER JOIN sparcsn4.inv_goods ON sparcsn4.inv_goods.gkey=sparcsn4.inv_unit.goods
WHERE sparcsn4.vsl_vessel_visit_details.ib_vyg='$ddl_imp_rot_no' AND inv_goods.destination=2592 AND inv_unit_equip.eq_role='PRIMARY'";
	$mlo_query_2=mysqli_query($con_sparcsn4,$mlo_query_2);
	$mlo_2=0;	
	while($mlo_row_2=mysqli_fetch_object($mlo_query_2)){
	$mlo_2++;


$summary_query_2="SELECT
IFNULL(SUM(20_86),0) AS icd_20_86,
IFNULL(SUM(20_96),0) AS icd_20_96,
IFNULL(SUM(40_86),0) AS icd_40_86,
IFNULL(SUM(40_96),0) AS icd_40_96,
...
WHERE sparcsn4.vsl_vessel_visit_details.ib_vyg='$ddl_imp_rot_no' AND ref_bizunit_scoped.id ='$mlo_row_2->id' AND inv_goods.destination=2592 AND inv_unit_equip.eq_role='PRIMARY') AS tmp";
	$summary_query_2=mysqli_query($con_sparcsn4,$summary_query_2);
	$sum=0;	
	while($summary_row_2=mysqli_fetch_object($summary_query_2)){
	$sum++;
