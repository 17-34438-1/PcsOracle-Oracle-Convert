...
/* 	SELECT igm_details.Import_Rotation_No,igm_masters.Vessel_Name,igm_detail_container.cont_number,igm_detail_container.cont_size,
			igm_detail_container.cont_height,IF(cont_size=20,1,IF((cont_size=40 OR cont_size=45),2,'')) AS teus, commudity_detail.commudity_desc,
			igm_detail_container.cont_gross_weight/1000 AS tonage, igm_details.Exporter_name, igm_details.Exporter_address, igm_details.Notify_name,
			igm_details.NotifyDesc,igm_details.Description_of_Goods */
    $total = 0;
	...
	/* 	$sql_igm_info = "SELECT igm_details.Import_Rotation_No,igm_masters.Vessel_Name,igm_detail_container.cont_number,igm_detail_container.cont_size,igm_detail_container.cont_height,
			...
			igm_details.Notify_name,igm_details.Description_of_Goods 			
			...
			WHERE igm_detail_container.cont_number IN('$cont_no') AND
			DATE(igm_details.file_clearence_date) BETWEEN '$goodsfromdate' AND '$goodstodate' AND UPPER(Description_of_Goods) LIKE UPPER('%SUGAR%') 
			UNION ALL 

			SELECT igm_supplimentary_detail.Import_Rotation_No,igm_masters.Vessel_Name,igm_sup_detail_container.cont_number,igm_sup_detail_container.cont_size,igm_sup_detail_container.cont_height,
			commudity_detail.commudity_desc,
			SUM(igm_sup_detail_container.cont_gross_weight)/1000 AS tonage, 
			igm_supplimentary_detail.Notify_name,igm_supplimentary_detail.Description_of_Goods  
			...
			WHERE  igm_sup_detail_container.cont_number IN('$cont_no') AND DATE(igm_supplimentary_detail.file_clearence_date)
			BETWEEN '$goodsfromdate' AND '$goodstodate' AND UPPER(Description_of_Goods) LIKE UPPER('%SUGAR%')";
		//return; */
		 if($search_value)		// importer
		{
			...
			INNER JOIN commudity_detail ON commudity_detail.`commudity_code`=igm_detail_container.`commudity_code`
			WHERE igm_detail_container.commudity_code IN('$item')  AND Notify_name LIKE '%$search_value%'
