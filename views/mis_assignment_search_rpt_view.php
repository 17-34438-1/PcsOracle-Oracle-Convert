<html>
	...
	<body>
		...
		include("dbOracleConnection.php");	
			...
			if($login_id=="admin")
			{
				@$strAssignment= "SELECT cont_id,BL_No,rotation,mfdch_value,propose_date,mis_assignment_entry.unit_gkey,phone_number,
				IFNULL(remarks,'') AS remarks,updated_by,snx_stat,mis_assignment_entry.consignee
				FROM ctmsmis.mis_assignment_entry 
				where snx_stat=0 AND ".$cond."
			...
			$name="";
			$sqlQuery="select ref_bizunit_scoped.name
			from
			ref_bizunit_scoped where ref_bizunit_scoped.id='$consignee'";
