...
<tr bgcolor="#ffffff" align="center" height="100px">
		<td colspan="13" align="center">
			<table border=0 width="100%">				
				<tr align="center">
					<td colspan="12"><img height="100px" src="<?php echo ASSETS_WEB_PATH;?>fimg/cpaLogo.png" /></td>
				</tr>
				<tr align="center">
...
<table width="100%" border ='1' cellpadding='0' cellspacing='0' >
	...
	<tr align="center">			
		...
		<td>
			<?php 
				...
				$resJRSC = mysqli_query($con_ctmsmis,$strJRSC);
				$nVal = 0;
				$plusVal = 0;
				while($rowJRSC = mysqli_fetch_object($resJRSC)){
					...
					$resCSJS = mysqli_query($con_ctmsmis,$strChkShareJrSc);
					$numRowCSJS = mysqli_num_rows($resCSJS);
					if($numRowCSJS>1)
					{
						$plusVal +=1;
		...
		<td>
			<?php 
				...
				$resABSC = mysqli_query($con_ctmsmis,$strABSC);
				$nValAB = 0;
				$plusValAB = 0;
				$numRowCSAS = 0;
				$strChkShareABSc = "";
				...
				while($rowABSC = mysqli_fetch_object($resABSC)){
					...
					if(!$inarr)
					{
						...
						$resCSAS = mysqli_query($con_ctmsmis,$strChkShareABSc);
						$numRowCSAS = mysqli_num_rows($resCSAS);
						if($numRowCSAS>1)
						{
							$plusValAB +=1;
	...
	<tr align="center">			
		...
		<td>
			<?php 
				// the following query had "AND eequipment LIKE 'FLT%'" which was creating an error
				// "equipment" instead of "eequipment" is used now
				$strABSC = "SELECT DISTINCT equipment
				FROM ctmsmis.mis_equip_assign_detail
				INNER JOIN ctmsmis.mis_equip_detail ON mis_equip_assign_detail.equip_detail_id=mis_equip_detail.id
				INNER JOIN ctmsmis.yard_block ON ctmsmis.yard_block.block=ctmsmis.mis_equip_assign_detail.block
				WHERE start_state =1 AND DATE(start_work_time)=DATE(NOW()) AND equipment LIKE 'FLT%' AND capacity=10 AND block_cpa='AB' ORDER BY 1";
				$resABSC = mysqli_query($con_ctmsmis,$strABSC);
				$nValAB = 0;
				$nValAB = 0;
				$plusValAB = 0;
...
<table width="100%" border ='1' cellpadding='0' cellspacing='0' >
			...
			$scQuery="SELECT
					...
					(SELECT IFNULL(non_operational,0) AS non_operational FROM ctmsmis.equip_assign_detail WHERE workshop_zone='C' AND equipment='SC') AS  
					c_tot_non_op";
			$rowSCQry=mysqli_query($con_ctmsmis,$scQuery);
			...
			$scQuery="SELECT
					...
					(SELECT IFNULL(non_operational,0) AS non_operational FROM ctmsmis.equip_assign_detail WHERE workshop_zone='C' AND equipment='FLT 16 Ton') AS  
					c_tot_non_op";
			$rowSCQry=mysqli_query($con_ctmsmis,$scQuery);
			while($rtnSCQuery=mysqli_fetch_object($rowSCQry))
			{
			...
			$scQuery="SELECT
					(SELECT IFNULL(equip_num,0) AS equip_num FROM ctmsmis.equip_assign_detail WHERE workshop_zone='AB' AND equipment='FLT 10 Ton') AS  ab_tot,
					(SELECT IFNULL(equip_num,0) AS equip_num FROM ctmsmis.equip_assign_detail WHERE workshop_zone='C' AND equipment='FLT 10 Ton') AS  c_tot,
					(SELECT IFNULL(non_operational,0) AS non_operational FROM ctmsmis.equip_assign_detail WHERE workshop_zone='AB' AND equipment='FLT 10 Ton') AS  ab_tot_non_op,
					(SELECT IFNULL(non_operational,0) AS non_operational FROM ctmsmis.equip_assign_detail WHERE workshop_zone='C' AND equipment='FLT 10 Ton') AS  
					c_tot_non_op";
			$rowSCQry=mysqli_query($con_ctmsmis,$scQuery);
			while($rtnSCQuery=mysqli_fetch_object($rowSCQry))
			{
