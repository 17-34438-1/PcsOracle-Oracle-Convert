...
<input type="hidden" name="txt_ROTATION" value="<?php print($row_rotationno->Import_Rotation_No); ?>">			
						<select name="txt_Org_Id_for_search2"  maxlength="50">
							<?php
								$resultcombo2 = mysqli_query($con_cchaportdb,"SELECT id,Organization_Name  FROM organization_profiles where Org_Type_id=2 ORDER BY Organization_Name");
...
<input type="hidden" name="txt_ROTATION" value="<?php print($row_rotationno->Import_Rotation_No); ?>">	
			...
			<b>IGM&nbsp;Submitted&nbsp;by&nbsp;SAF:	</b>
				<select name="txt_Org_Id_for_search122"  maxlength="50" onFocus="gsLabelObj(lbl_org_id122,'#61BCEF','white')" onBlur="gsLabelObj(lbl_org_id122,'','')">
					<?php
						$resultcombo1 = mysqli_query($con_cchaportdb,"SELECT distinct(Submitee_Org_Id) as id,(select Organization_Name from organization_profiles orgs where orgs.id=igm_details.Submitee_Org_Id) as Organization_Name 
FROM  igm_details where IGM_id=$this->CODE and Substring(Submitee_Id,-1)='F' and type_of_igm='$this->TM' ORDER BY Organization_Name");
						while ($rowcombo1 = mysqli_fetch_object($resultcombo1)){
					?>
...
<input type="hidden" name="txt_ROTATION" value="<?php print($row_rotationno->Import_Rotation_No); ?>">	
		<b>	IGM&nbsp;Submitted&nbsp;by&nbsp;MLO:		</b>
				<select name="txt_Org_Id_for_search123"  maxlength="50" onFocus="gsLabelObj(lbl_org_id123,'#61BCEF','white')" onBlur="gsLabelObj(lbl_org_id123,'','')">
					<?php
						$resultcombo = mysqli_query($con_cchaportdb,"SELECT distinct(Submitee_Org_Id) as id,(select Organization_Name from organization_profiles orgs where orgs.id=igm_details.Submitee_Org_Id) as Organization_Name 
