...
<table border="0" width="30%">
	...
	<TD align="center"><?php 
		...
		//print($str);
		$result_rotationno = mysqli_query($con_cchaportdb,$str);
		$row_rotationno=mysqli_fetch_object($result_rotationno);
...
?>			
			...
			Search&nbsp;for&nbsp;FF:		
				<select name="txt_Org_Id_for_search"  maxlength="50" onFocus="gsLabelObj(lbl_org_id12,'#61BCEF','white')" onBlur="gsLabelObj(lbl_org_id12,'','')">
					<?php
						$resultcombo = mysqli_query($con_cchaportdb,"SELECT id,Organization_Name  FROM organization_profiles where Org_Type_id=4 ORDER BY Organization_Name");
						while ($rowcombo = mysqli_fetch_object($resultcombo)){
					?>
...
<td><b>Search&nbsp;for&nbsp;Port:</b></td>
		<td>
				<!--<form action="home.php" method="post">-->
			<?php
				echo form_open('IgmViewController/myListSearchforPort');
...
<table border="0" width="2000px">
		<tr>			
			...
			<!--<form action="home.php" method="post">-->
			<?php
				echo form_open('IgmViewController/myListSearchBySAF');
				$Stylepadding = 'style="padding: 12px 20px;"';
				if(!empty($error_message))
				{
					$Stylepadding = 'style="padding:25px 20px;"';
...
<td  id="lbl_org_id123"  class="onlyTable1">
			<!--<form action="home.php" method="post">-->
			<?php
				echo form_open('IgmViewController/myListSearchByMLO');
				$Stylepadding = 'style="padding: 12px 20px;"';
				if(!empty($error_message))
				{
					$Stylepadding = 'style="padding:25px 20px;"';
			...
			<b>Search&nbsp;by&nbsp;MLO&nbsp;CODE:	</b>	
				...
				<select name="txt_Org_Id_for_search123"  maxlength="50" onFocus="gsLabelObj(lbl_org_id124,'#61BCEF','white')" onBlur="gsLabelObj(lbl_org_id124,'','')">
					<?php
						...
						$resultcombo = mysqli_query($con_cchaportdb,"SELECT distinct(mlocode) 
...
<table>
		...
		<td>Consignee&nbsp;and&nbsp;Notify&nbsp;Party</td><td>Bill&nbsp;Of&nbsp;Entry&nbsp;Number</td><td>Bill&nbsp;Of&nbsp;Entry&nbsp;Date</td>
		<td>Truck&nbsp;Entry</td><td>Delivered</td><td>Discharged</td><td>C&nbsp;&&nbsp;F&nbsp;Agent&nbsp;Name</td><td>Remarks</td><td>AIR BLock Status</td><td>Delivery Block Status</td><td>Intelligence Block Status</td>
...
//include("DatasoftUtility.php");
		...
		?>
			<?php 
				...
				{
				?>
				<!-- <tr style="background-color:#E1F0FF" align="center"> -->
...
//print($str);
$result1 = mysqli_query($con_cchaportdb,$str);						
					...
					// load consignee
						...
						$result2 = mysqli_query($con_cchaportdb,"select cons.id,cons.igm_detail_id,cons.Consignee_ID,(select org.Organization_Name from organization_profiles org where org.id=cons.Consignee_ID) as consignee_name,(select org1.Address_1 from organization_profiles org1 where org1.id=cons.Consignee_ID) as Address_1,cons.ff_clearance as ff_clearance from igm_detail_consigneetabs cons where cons.igm_detail_id=$id");						
		...
		$resprior = mysqli_query($con_cchaportdb,"select PFStatus from igm_details where id=$igmMasterList[$i]['id']");
			if($rowprior = mysqli_fetch_object($resprior)) 
				{
			...
			/// End IGM Acceptance Status
					...
					// load notify
						...
						$result3 = mysqli_query($con_cchaportdb,"select notf.id,notf.igm_detail_id,notf.Notify_ID,(select org.Organization_Name from organization_profiles org where org.id=notf.Notify_ID) as notify_name,(select org1.Address_1 from organization_profiles org1 where org1.id=notf.Notify_ID) as Address_1,notf.ff_clearance as ff_clearance from igm_detail_notifytabs notf where notf.igm_detail_id=$id");						
...
<td valign="top" align="center">
	<form method="POST" action="<?php echo site_url("IgmViewController/shedDeliveryOrderInfoData") ?>">
		<input type="hidden" name="type_of_Igm" id="type_of_Igm" value="<?php echo $type;?>">
		<input type="hidden" name="CODE" id="CODE" value="<?php echo $CODE;?>">
		<input type="hidden" name="blno" id="blno" value="<?php echo $igmMasterList[$i]['BL_No'];?>">
		<input type="hidden" name="rotno" id="rotNo" value="<?php echo $igmMasterList[$i]['Import_Rotation_No'];?>">
		<button type="submit">DO Entry</button>
	</form>
	<?php 
		$mycnt =0;
		$z =0;
		$zRot ="";
		$zBl ="";
		$thisbl = $igmMasterList[$i]['BL_No'];
		$thisrot = $igmMasterList[$i]['Import_Rotation_No'];
		$fileRotno=str_replace('/','_',$thisrot);
		$queryInfo = mysqli_query($con_cchaportdb,"SELECT * FROM do_info_image WHERE imp_rot='$thisrot' AND bl_no='$thisbl' ORDER BY id DESC");
		while($rowQueryInfo = mysqli_fetch_object($queryInfo)) {
			$mycnt++;
		}
	?>
	<?php for ($x=$mycnt;$x>0;$x=$x-1){ 
		$queryRotBl = mysqli_query($con_cchaportdb,"SELECT id,gross_quantity,delv_quantity 
					FROM do_info_image 
					WHERE do_info_image.imp_rot='$thisrot' AND do_info_image.bl_no='$thisbl' 
					ORDER BY id DESC LIMIT $z,1");
		$rowQueryRotBl = mysqli_fetch_object($queryRotBl);
		$doImgInfoId = $rowQueryRotBl->id;
	?>
		<span>
			<a href="<?php echo site_url("IgmViewController/DeliveryOrderBBImgShow/".$z."/".$thisbl."/".$fileRotno) ?>" target="_BLANK" style="color:blue;">
				Image <?php echo $x;?>
			</a> 
			/ 
			<a href="<?php echo site_url("IgmViewController/DeliveryOrderBBPDFShow/".$thisbl."/".$fileRotno."/".$doImgInfoId) ?>" 
	...
	<?php $z++; } ?>
	<!--form method="POST" action="<?php echo site_url("IgmViewController/DeliveryOrderBBImgShow") ?>" target="_BLANK">
		<input type="hidden" name="blno" id="blno" value="<?php echo $igmMasterList[$i]['BL_No'];?>">
		<input type="hidden" name="rotno" id="rotno" value="<?php echo $igmMasterList[$i]['Import_Rotation_No'];?>">
		<button type="submit">DO Image</button>
	</form>
		
	<form method="POST" action="<?php echo site_url("IgmViewController/DeliveryOrderBBPDFShow") ?>" target="_BLANK">
		<input type="hidden" name="blno" id="blno" value="<?php echo $igmMasterList[$i]['BL_No'];?>">
		<input type="hidden" name="rotno" id="rotno" value="<?php echo $igmMasterList[$i]['Import_Rotation_No'];?>">
		<button type="submit">DO PDF</button>
	</form-->
	<br><hr>
	
	
	<a href='Forms/myBillEntryImportReportHTML.php?reg=<?php print($igmMasterList[$i]['Bill_of_Entry_No']);?>&date=<?php print($igmMasterList[$i]['Bill_of_Entry_Date']);?>&code=<?php print($igmMasterList[$i]['office_code']);?>' target="aboutblank"><?php print($igmMasterList[$i]['Bill_of_Entry_No']);?></a>
...
<?php if($igmMasterList[$i]['R_No'])  print($igmMasterList[$i]['R_No']); else print("&nbsp;"); print("<br>");   ?>
					...
					<td valign="top"><?php if($igmMasterList[$i]['Bill_of_Entry_No']=='') { ?>
					<?php
					print("<hr noshade>"); 
						// $igmMasterList[$i]['Bill_of_Entry_No'] = "GHYT5";
						$billOfEntry = $igmMasterList[$i]['Bill_of_Entry_No'];
						$beCount = mysqli_query($con_cchaportdb,"SELECT COUNT(*) AS rtnValue,do_image_loc FROM do_image WHERE be_no = '$billOfEntry'");
						$row5 = mysqli_fetch_object($beCount);
						if($row5->rtnValue > 0){
					?>

						<a href="../../assets/do_image/<?php echo $row5->do_image_loc; ?>" target="_blank">View DO</a>

					<?php //}else if($row5->rtnValue == 0 and $_SESSION['Control_Panel'] == 57){ ?>
					<?php }else if($row5->rtnValue == 0 or $_SESSION['Control_Panel'] == 57){ ?>

						<!--button onclick="showTruckModal()">Truck Entry</button-->
						<form method="POST" name="truckEntry" id="truckEntry" action="<?php echo site_url('igmViewController/truckEntryforDeliveryOrderForm') ?>" target="_blank">
						
							<input type="hidden" style="width:150px;" id="billOfEntry" name="billOfEntry" value="<?php  echo $igmMasterList[$i]['Bill_of_Entry_No'];?>" />
							<input type="hidden" id="bl_no_bb" name="bl_no_bb" value="<?php echo $igmMasterList[$i]['BL_No'];?>">
							<input type="hidden" id="rot_no_bb" name="rot_no_bb" value="<?php echo $igmMasterList[$i]['Import_Rotation_No'];?>">
							<input type="hidden" id="typeOfIgm" name="typeOfIgm" value="<?php echo $type;?>">
							
							<button>Truck Entry</button>
						</form>
					<?php } ?>
					
					<table>
						<?php 	
							$bbRot=$igmMasterList[$i]['Import_Rotation_No'];
							$bbBL=$igmMasterList[$i]['BL_No'];
						 $truckStr = "SELECT truck_id,delv_pack FROM do_bb_truck_details_entry WHERE 
									import_rotation='$bbRot' AND bl_no='$bbBL'";				
						$rsltTruckStr = mysqli_query($con_cchaportdb,$truckStr);
						//echo 
						if(mysqli_num_rows($rsltTruckStr)>0)
						{ 	
							?>			
							<tr><td class='gridLight'> Truck</td class='gridLight'> <td>Quantity</td></tr>
							<?php
							while($row = mysqli_fetch_object($rsltTruckStr)) 
							{
								?>
								
								<tr><td class='gridDark'><?php echo $row->truck_id; ?></td class='gridDark'> <td><?php echo $row->delv_pack; ?></td></tr>
							<?php
							}
						}
					...
					// load CnF
						...
						$igm_detail_id = $igmMasterList[$i]['id'];
                        $result4 = mysqli_query($con_cchaportdb,"select cnf.id,cnf.igm_detail_id,cnf.CnF_ID_to_be_AccountedFor 
	...
	where cnf.igm_detail_id='$igm_detail_id'");				
...
if($_SESSION['Control_Panel']==14)
		...
		?>
<TR><TD colspan="2" align="center"><p><?php echo $links; ?></p></TD></TR>
</table>
