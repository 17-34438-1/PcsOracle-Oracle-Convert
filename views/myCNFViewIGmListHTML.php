    ...
    <div class="content_resize">
      <div class="mainbar">
		...
		<table border="0" width="100%">
			...
			<TR align="left"><TD colspan="6" ><a href="<?php echo site_url('FrontEndController/Dashboard') ?>"><img src="<?php echo IMG_PATH; ?>back.png" height="40px" width="40px" align="middle" hspace="5"/>Back to Control Panel</a></TD></TR>
		...
		<?php
            ...
            for($i=0;$i<$len;$i++)
			{
				...
				// $row_edi_file_name=mysqli_fetch_assoc($rslt_edi_file_name);
				
				$edi_file_name = "";
				$stow_file_name = "";
				while($row_edi_file_name=mysqli_fetch_object($rslt_edi_file_name))
				{
					$edi_file_name=$row_edi_file_name->file_name_edi;
					$stow_file_name=$row_edi_file_name->file_name_stow;
				}
				
				// $edi_file_name=@$row_edi_file_name[0]['file_name_edi'];
				// $stow_file_name=@$row_edi_file_name[0]['file_name_stow'];
				...
				$strVvd = "SELECT vvd_gkey FROM vsl_vessel_visit_details
				WHERE ib_vyg='$myrot'";
				$resVvd= oci_parse($con_sparcsn4_oracle,$strVvd);
                oci_execute($resVvd);
				$results=array();
				$numrows = oci_fetch_all($resVvd, $results, null, null, OCI_FETCHSTATEMENT_BY_ROW);
				oci_free_statement($resVvd);

				$resVvd= oci_parse($con_sparcsn4_oracle,$strVvd);
                oci_execute($resVvd);
				$rowVvd = oci_fetch_object($resVvd);
		...
		?>
			    <tr class="gridLight">
					...
					if($_SESSION['Control_Panel']==57 or $_SESSION['Control_Panel']==12 or $_SESSION['Control_Panel']==28 or 
						$_SESSION['Control_Panel']==30 or $_SESSION['Control_Panel']==1)
					{
						if($_SESSION['Control_Panel']==57 or $_SESSION['Control_Panel']==28 or $_SESSION['Control_Panel']==1)
						{
					?>
			 ...
			 <?php
		      ...
		      <a href='home.php?myflag=6194&CODE=<?php print($row->id);?>&SUBCODE=<?php print($row->Import_Rotation_No);?>&TM=<?php print($this->VD); ?>' style="color:blue">Upload IGM Sub(GM) from Excel File</a><br><hr>
					...
					if($_SESSION['Control_Panel']==6 or $_SESSION['Control_Panel']==12 or $_SESSION['Control_Panel']==13 or $_SESSION['Control_Panel']==43 or $_SESSION['Control_Panel']==14 or $_SESSION['Control_Panel']==59)
					{		
					?>
					...
					<?php
					//if($_SESSION['Control_Panel']==7 or $_SESSION['Control_Panel']==6 or $_SESSION['Control_Panel']==12 or $_SESSION['Control_Panel']==13 or $_SESSION['Control_Panel']==10 or $_SESSION['Control_Panel']==44 or $_SESSION['Control_Panel']==58 or $_SESSION['Control_Panel']==30)
					//{
					//for egm general manifest
					if($type=='GM'){
					?>
