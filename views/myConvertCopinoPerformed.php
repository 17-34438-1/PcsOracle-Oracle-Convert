<?php
	...
	$strVslName = "select name,argo_carrier_visit.id from vsl_vessel_visit_details
	inner join argo_carrier_visit on argo_carrier_visit.cvcvd_gkey=vsl_vessel_visit_details.vvd_gkey
	inner join vsl_vessels on vsl_vessels.gkey=vsl_vessel_visit_details.vessel_gkey
	where ib_vyg='$rotation_no' order by argo_carrier_visit.gkey desc FETCH FIRST 1 ROWS ONLY";
	// $resVslName = mysqli_query($con_sparcsn4,$strVslName);
	// $rowVslName = mysqli_fetch_object($resVslName);



	$resVslName = oci_parse($con_sparcsn4_oracle, $strVslName);


	$rowVslName=oci_execute($resVslName);
	...
	$resAgent = mysqli_query($con_sparcsn4,$strAgent);
	
	$agent="";
	while($rowAgent= mysqli_fetch_object($resAgent))
	{
	...
	while($rowMain= mysqli_fetch_object($resMain))
	{
		...
		$strPod = "select place_code,place_name from ref_unloc_code where id='$rowMain->pod'";
		$resPod = mysqli_query($con_sparcsn4,$strPod);
		$rowPod = mysqli_fetch_object($resPod);
		...
		*/
		$strTrans = "select inv_unit_fcy_visit.transit_state,inv_unit.category,argo_carrier_visit.carrier_mode
		from inv_unit 
		...
		$resTrans=oci_execute($resVslName);
		...
		while(($rowTrans=oci_fetch_object($resVslName))!= false)
		...
		$resCommodity = mysqli_query($con_sparcsn4,$strCommodity);
		
		// $rowCommodity = mysqli_fetch_object($resCommodity);
		$commudity_desc = "";
		while($rowCommodity = mysqli_fetch_object($resCommodity))
		{
		...
		if($s==0)
		{
			$strUpdateCont = "update inv_unit set foreignhost_key=NULL where id='$rowMain->cont_id' and category='EXPRT'";
			mysqli_query($con_sparcsn4,$strUpdateCont);
	...
	$END = $END.'</argo:snx>'."\n";
	//echo $END;
	// $file_old = $agent."-".$rotation_no."-".$vslName;
	$file_old = $agent."-".$vslName;
	$myFile_old="COPINO-".$file_old.'.xml';	
	if(file_exists($_SERVER['DOCUMENT_ROOT']."/resources/copino/".$myFile_old))
	{		
		unlink($_SERVER['DOCUMENT_ROOT']."/resources/copino/".$myFile_old);
	}
	$fh= fopen($_SERVER['DOCUMENT_ROOT']."/resources/copino/".$myFile_old , 'a');
	fwrite($fh, $UNB);		
	fwrite($fh, $NAD);
	fwrite($fh, $END);
	fclose($fh);
	...
	if (file_exists($_SERVER['DOCUMENT_ROOT']."/resources/copino/".$myFile_old))
	{
		$file = $_SERVER['DOCUMENT_ROOT']."/resources/copino/".$myFile_old;
		$fp = fopen($file, 'rb');
