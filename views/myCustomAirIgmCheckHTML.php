...
$sqloffice=mysqli_query($con_cchaportdb,"select office_code from users where login_id='$logid'");
$rowoffice=mysqli_fetch_object($sqloffice);
...
if ((@$row->amendment_appoved<>1) and ($row->PFstatus=1 or $row->PFstatus=10)) {
	...
	<?php
		...
		$con_num_row=mysqli_num_rows($result1);
		?>
		<td><?php print($i); ?></td>
		<td><?php print($rotation = $row->Import_Rotation_No); ?></td>
		<td><?php print($row->Organization_Name); ?></td>
		<td><?php print($row->mlocode); ?></td>
		<td><?php print($row->Line_No); ?></td>
		<td><?php print($bl = $row->BL_No); ?></td>
				...
				<table width="100%">
					...
					<?php 
						...
						while($row1 = mysqli_fetch_object($result1)) {
							...
							<td>
							<?php 
								$sql_offdock=mysqli_query($con_cchaportdb,"select Organization_Name from organization_profiles where id='$row1->off_dock_id'");
								$row_offdock=mysqli_fetch_object($sql_offdock);
		...
		<?php
						...
						if(@$row_consignee->SAD_CONSIGNEE)
								{
									$sql_consignee=mysqli_query($con_cchaportdb,"select CMP_NAM,
								...
								{
									$sql_consignee=mysqli_query($con_cchaportdb,"select SAD_CON_NAM as CMP_NAM,concat(SAD_CON_ADD1,SAD_CON_ADD2) as address 
			...
			<td align="center">
			<!-- <a href='Forms/myBillEntryImportReportHTML.php?reg=<?php //print($row->Bill_of_Entry_No);?>&date=<?php //print($row->Bill_of_Entry_Date);?>&code=<?php //print($row->office_code);?>' target="aboutblank"><?php //print($row->Bill_of_Entry_No); ?>
			</a> -->
			<?php
				$beno = $row->Bill_of_Entry_No;
		...
		<td>
		<?php  
		$sqligmmaster=mysqli_query($con_cchaportdb,"select Port_of_Shipment from igm_masters where Import_Rotation_No='$row->Import_Rotation_No'");
		$rowigmmaster=mysqli_fetch_object($sqligmmaster);	
