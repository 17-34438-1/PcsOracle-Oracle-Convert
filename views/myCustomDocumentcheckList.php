...
{
	$sqlMaster=mysqli_query($con_cchaportdb,"select file_clearence_date from igm_masters where Import_Rotation_No='$txt_imp_rot1'");
	$rowMaster=mysqli_fetch_object($sqlMaster);
...
','')='$txt_bl' and final_submit='1'
		...
		//	print($sql."<hr>");
			$res_cnt=mysqli_query($con_cchaportdb,$sql);
...
$sqlMaster=mysqli_query($con_cchaportdb,"select file_clearence_date from igm_masters where Import_Rotation_No='$txt_imp_rot'");
	$rowMaster=mysqli_fetch_object($sqlMaster);
	//print($rowMaster->file_clearence_date."<hr>");
	/*if(!($_SESSION['Control_Panel']==10 or $_SESSION['Control_Panel']==12 or $_SESSION['Control_Panel']==44)){
	if($rowMaster->file_clearence_date>"2013-09-02")
...
<th><span class="style1">Vessel_Name</span></th>
						...
						<tr>
							<?php while(@$row1=mysqli_fetch_object($res_cnt)){ 
...

{
	...
	fclose($handle111); 
				...
				//****************end**************
					...
					$str_vessel=mysqli_query($con_cchaportdb,"select Vessel_Name,file_clearence_logintime,file_clearence_date,final_clerance_files_ref_number from igm_masters where id='$row1->master_id'");
...
//****************end**************
			...
			<td  align="center"><?php if($row1->Import_Rotation_No)  print($row1->Import_Rotation_No); else print("&nbsp;"); ?></td>
				<?php $strin1=mysqli_query($con_cchaportdb,"SELECT (SUBSTRING_INDEX('$row1->Line_No', '*', -1)) as Line_No,(SUBSTRING_INDEX('$row1->BL_No', '*', -1)) as BL_No");
			//print("SELECT (SUBSTRING_INDEX('$row1->Line_No', '*', -1)) as Line_No,(SUBSTRING_INDEX('$row1->BL_No', '*', -1)) as BL_No");
			$line_bl=mysqli_fetch_object($strin1);?>
...
//$Sad_reg_no=substr($row1->Bill_of_Entry_No,1);
					  $bin=mysqli_query($con_cchaportdb,"select  id,SAD_CONSIGNEE,SAD_DECLARANT,org_id from sad_gen_informations where  SAD_REG_NBER='$Sad_reg_no' 
and SAD_REG_DATE='$row1->Bill_of_Entry_Date' and SAD_CUO_COD='$row1->office_code'");
	$rowbin = null;
	if(!is_bool($bin)){
		$rowbin=mysqli_fetch_object($bin);
	} 

	if(count($rowbin)>0){
		if($rowbin->SAD_DECLARANT<>0)
		{
		$org_name=mysqli_query($con_cchaportdb,"select Organization_Name,block_open,block_open_by,block_open_comment,block_date from organization_profiles where id='$rowbin->org_id'");
		...
		$roworg=mysqli_fetch_object($org_name); 
...
$sql_organization=mysqli_query($con_cchaportdb,"select Organization_Name as Organization_Name from organization_profiles orgs where id='$row1->Submitee_Org_Id'");
//print("select Organization_Name as Organization_Name from organization_profiles orgs where id='$row1->Submitee_Org_Id'");
$row_organization=mysqli_fetch_object($sql_organization);
...
?>
					...
					<td  align="center">
						<!-- <?php //if($beno)  { ?><a href='Forms/myBillEntryImportReportHTML.php?reg=<?php //print($be);?>&date=<?php //print($BEdate);?>&code=<?php //print($office);?>' target="aboutblank"><?php //print($beno); ?></a>print($row1->Bill_of_Entry_No);<?php //print("<font color=red size=2><hr> {$msg1}</font>"); }else print("&nbsp;"); ?> -->
						<?php
							$rotation = $txt_imp_rot;
							if(strlen($rotation) == 0){
								$rotation = $txt_imp_rot1;
							}
							...
							$sad_manif = str_replace("/"," ",$rotation);
							$sad_info_query = "SELECT office_code,reg_no,reg_date,place_dec,entry_dt,dec_code
							FROM sad_info 
							INNER JOIN sad_item ON sad_info.id = sad_item.sad_id
							WHERE sad_info.manif_num = '$sad_manif' AND sad_item.sum_declare = '$bl'";
							$sad_rslt = $this->bm->dataSelectDb1($sad_info_query);
							$office_code = "";
							$c_number = "";
							$xmlDate = "";

							if(count($sad_rslt)>0){
								$office_code = $sad_rslt[0]['office_code'];
								$c_number = $sad_rslt[0]['reg_no'];
								$xmlDate = $sad_rslt[0]['reg_date'];
							}
						?>

						<?php if($beno)  { ?>
						<form action="<?php echo site_url("report/xml_conversion_action");?>" method="post" target="_blank">
							<input type="hidden" name="office_code" id="office_code" value="<?php echo $office_code; ?>">
							<input type="hidden" name="c_nubmber" id="c_nubmber" value="<?php echo $c_number; ?>">
							<input type="hidden" name="xml_date" id="xml_date" value="<?php echo $xmlDate; ?>">
							<input type="submit" name="view" id="view" style="width:50px;color:blue;cursor:pointer;" value="<?php print($beno); ?>" class="btn btn-primary btn-sm">
						</form>
						<?php print("<font color=red size=2><hr> {$msg1}</font>"); }else print("&nbsp;"); ?>
					</td>
				...
				<td  align="center"><?php if($row1->R_No)  print($row1->R_Date); else print("&nbsp;"); ?></td>-->
					...
					<td  align="center">
						...
						<?php if($row1->R_No)  print($row1->R_No); else print("&nbsp;"); ?>
						</a>
					</td>

					<td  align="center"><?php if($row1->R_No)  print($row1->R_Date); 
						else print("&nbsp;"); ?>
					</td>
					...
					<td>
						...
						$sql_consignee=mysqli_query($con_cchaportdb,"select SAD_CONSIGNEE,SAD_TYP_DEC 
						...
						$numrowcon=mysqli_num_rows($sql_consignee);
						if($numrowcon==0)
						{
							$sql_consignee=mysqli_query($con_cchaportdb,"select SAD_CONSIGNEE,SAD_TYP_DEC 
						...
						SAD_REG_DATE='$row1->Bill_of_Entry_Date' and SAD_CUO_COD=$row1->office_code");
*/
						$row_consignee=mysqli_fetch_object($sql_consignee);
			...
			if($row1->igm_type=='D') 
			{
				$office_afr= mysqli_query($con_cchaportdb,"select block_status,office_code,AFR_By,AFR_Date,remarks from igm_detail_afr where igm_detail_id='$row1->id'  
				and AFR_By is not null order by AFR_Date desc limit 2");
			}
			else
			{
				$office_afr= mysqli_query($con_cchaportdb,"select block_status,office_code,AFR_By,AFR_Date,remarks from igm_detail_afr where igm_sup_detail_id='$row1->id'  
				and AFR_By is not null order by AFR_Date desc limit 2");
				
			}
			$numrowAIR=mysqli_num_rows($office_afr);
			...
			if($row1->igm_type=='D') 
			{
				$office= mysqli_query($con_cchaportdb,"select block_status,office_code,AFR_By,AFR_Date,Deliver_block_by,remarks,Deliver_block_time from igm_detail_afr where igm_detail_id='$row1->id' and  Deliver_block_by is not null 
				order by Deliver_block_time desc limit 2");
			}
			else
			{
				$office= mysqli_query($con_cchaportdb,"select block_status,office_code,AFR_By,AFR_Date,Deliver_block_by,remarks,Deliver_block_time from igm_detail_afr where 
				igm_sup_detail_id='$row1->id' and  Deliver_block_by is not null order by Deliver_block_time desc limit 2");
			}
			//$numrowDIV=mysqli_num_rows($office);
			...
			<td  align="center">
					...
					if($row1->igm_type=='D')
					$sql_int_block=mysqli_query($con_cchaportdb,"select block_status,int_block_status,int_block_datetime,remarks from igm_detail_afr where igm_detail_id=$row1->id and int_block_status is not null order by id desc limit 1");
					else 
					$sql_int_block=mysqli_query($con_cchaportdb,"select block_status,int_block_status,int_block_datetime,remarks from igm_detail_afr where igm_sup_detail_id=$row1->id and int_block_status is not null order by id desc limit 1");
				...
				<?php 
				@$sql_delivery=mysqli_query($con_cchaportdb,"select Port_Checked,Check_by_port,Check_datetime from delivery_dash_board where sad_gen_id=$rowbin->id");
				@$row_delivery=mysqli_fetch_object($sql_delivery);
			...
			//$str="select response_details3,response_details2,response_details1,hold_application,rejected_application,auto_no 			as submitId,final_amendment,appsubmitdate,navy_response_to_port,permission_no from igm_navy_response left join $table on $table.id=igm_navy_response.egm_details_id where igm_navy_response.egm_details_id=$row1->id";
					...
					//print($str);
			$resultnavy = mysqli_query($con_cchaportdb,$str);	
			$navy=mysqli_fetch_object($resultnavy);
...
where cnt.igm_detail_id=$row1->id";
						$result2 = mysqli_query($con_cchaportdb,$str2);						
...
$vessel_bearth=mysqli_query($con_cchaportdb,"select ETA_Date,Actual_Berth from vessels_berth_detail where igm_id='$row1->master_id'");
//print("select ETD_Date,Actual_Berth from vessels_berth_detail where igm_id='$row1->master_id'");
$row_bearth=mysqli_fetch_object($vessel_bearth);
