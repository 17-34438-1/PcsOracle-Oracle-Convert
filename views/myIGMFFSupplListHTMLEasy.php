...
<input type="hidden" name="txt_ROTATION" value="<?php print($row_rotationno->Import_Rotation_No); ?>">	
				...
				<select name="txt_Org_Id_for_cnfcc7"  maxlength="50" onFocus="gsLabelObj(lbl_org_id14,'#61BCEF','white')" onBlur="gsLabelObj(lbl_org_id14,'','')">
					<?php
						$resultcombo3 = mysqli_query($con_cchaportdb,"SELECT id,Organization_Name  FROM organization_profiles where Org_Type_id=5 or Org_Type_id=6 ORDER BY Organization_Name");
						while ($rowcombo3 = mysqli_fetch_object($resultcombo3)){
					?>
		...
		<tr>
			...
			<td  id="lbl_org_id17"  class="onlyTable1">
					...
					?>
						...
						<input type="hidden" name="MCODE" value="<?php print($this->MCODE); ?>">	
						<input type="hidden" name="master_id" value="<?php print(@$igmMasterList[0]['igm_master_id']); ?>">	
						<input type="hidden" name="TM" value="<?php print($this->TM); ?>">
						<input type="hidden" name="txt_SFlag" value="<?php print(@$this->SFlag); ?>">	
<input type="hidden" name="txt_ROTATION" value="<?php print($row_rotationno->Import_Rotation_No); ?>">
						<b>Search&nbsp;By:&nbsp;</b>
			...
			<?php
				...
				<select name="txt_Org_Id_for_submittedFF"  maxlength="50" onFocus="gsLabelObj(lbl_org_id133,'#61BCEF','white')" onBlur="gsLabelObj(lbl_org_id133,'','')">
					...
					{
					$resultcombo2 = mysqli_query($con_cchaportdb,"select distinct Submitee_Org_Id as id,organization_profiles.Organization_Name from igm_supplimentary_detail  igms 
					...
					$resultcombo2=mysqli_query($con_cchaportdb,"select id,Organization_Name from organization_profiles where Org_Type_id='4' order by Organization_Name");
...
if($_SESSION['Control_Panel']==14)
		...
		<?php
			    ...
			    <tr class='gridLight'>
					...
					<td valign="top"><?php print($igmMasterList[$i]['Agent_Name'].'<br>'.'AIN NO: '.$AIN_No); ?>...</td>
...
if($CType=="FF" or $CType=="FFnosubmit")
 $Table_List[3]="igm_sup_detail_container";
					$id=$igmMasterList[$i]['id'];
						...
						$result1 = mysqli_query($con_cchaportdb,"select cnt.id as id,cnt.cont_number as cont_number,cnt.cont_size as cont_size,cnt.cont_iso_type as cont_iso_type,cnt.cont_type 
...
cnt.off_dock_id=organization_profiles.id where cnt.igm_sup_detail_id=$id");						
					...
					<!--   below portion commented due to error  ($row)   -->
					<!-- <?php 
					// load notify
						...
						//print("notify:".$str);
						$result3 = mysqli_query($con_cchaportdb,$str);
		...
		// PORT
			if($this->TM=="GM")
			{
				...
				print("<td><a href='home.php?myflag=518&CODE=&TM=$this->TM'>Update Cont. Discharge Status</a><br><hr noshade><a href='home.php?myflag=521&CODE=&TM=$this->TM'>Add Cont. Unstaffing</a><br><hr noshade><a href='home.php?myflag=515&CODE=&TM=$this->TM'>Update Cont. Delivery Status</a></td>");  
			}
			else
			{
