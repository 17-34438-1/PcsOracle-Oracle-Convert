...
<?php
	...
	$query=oci_parse($con_sparcsn4_oracle,"SELECT DISTINCT * FROM ( SELECT a.id AS cont_no, (SELECT inv_goods.destination FROM inv_goods WHERE inv_goods.gkey=a.goods) AS destination,
	b.time_in AS dischargetime, b.time_out AS delivery, g.id AS mlo, a.flex_string01 AS assignmenttype, a.gkey, a.freight_kind AS statu, a.goods_and_ctr_wt_kg AS
	weight, ref_equip_type.iso_group AS iso_code, vsl_vessel_visit_details.ib_vyg AS rot_no, substr(ref_equip_type.nominal_height,-2) as hight,
	substr(ref_equip_type.nominal_length,-2) AS siz, (SELECT substr(srv_event_field_changes.new_value,7) FROM srv_event INNER JOIN srv_event_field_changes
	ON srv_event_field_changes.event_gkey=srv_event.gkey WHERE srv_event.applied_to_gkey=a.gkey AND srv_event.event_type_gkey IN(31450,31443,31447) AND
	srv_event_field_changes.new_value IS NOT NULL AND srv_event_field_changes.new_value !='' AND srv_event_field_changes.new_value !='Y-CGP-.' AND
	srv_event.gkey<(SELECT srv_event.gkey FROM srv_event INNER JOIN srv_event_field_changes ON srv_event_field_changes.event_gkey=srv_event.gkey WHERE
	srv_event.event_type_gkey IN(31473) AND srv_event.applied_to_gkey=a.gkey ORDER BY srv_event_field_changes.gkey DESC FETCH FIRST 1 ROWS ONLY) ORDER
	BY srv_event.gkey DESC FETCH FIRST 1 ROWS ONLY) AS carrentPosition, (SELECT srv_event.created FROM srv_event INNER JOIN srv_event_field_changes
	ON srv_event_field_changes.event_gkey=srv_event.gkey WHERE applied_to_gkey=a.gkey AND event_type_gkey=31426 AND srv_event_field_changes.new_value='E'
	FETCH FIRST 1 ROWS ONLY) AS proEmtyDate, b.time_out AS emptyDate, b.flex_date01 AS assignmentdate FROM inv_unit a INNER JOIN inv_unit_fcy_visit b
	ON b.unit_gkey=a.gkey INNER JOIN argo_carrier_visit ON argo_carrier_visit.gkey=a.declrd_ib_cv INNER JOIN argo_visit_details ON
	argo_visit_details.gkey=argo_carrier_visit.cvcvd_gkey INNER JOIN vsl_vessel_visit_details ON vsl_vessel_visit_details.vvd_gkey=argo_visit_details.gkey INNER
	JOIN ref_bizunit_scoped g ON a.line_op = g.gkey INNER JOIN config_metafield_lov ON a.flex_string01=config_metafield_lov.mfdch_value INNER JOIN inv_goods
	j ON j.gkey = a.goods LEFT JOIN ref_bizunit_scoped k ON k.gkey = j.consignee_bzu INNER JOIN ref_equipment ON ref_equipment.gkey=a.eq_gkey INNER JOIN
	ref_equip_type ON ref_equip_type.gkey=ref_equipment.eqtyp_gkey 
	WHERE (to_char(b.time_out,'yyyy-mm-dd') BETWEEN '$fromdate' AND '$todate') AND a.seal_nbr3='E' ) tmp order by emptyDate");
	oci_execute($query);
