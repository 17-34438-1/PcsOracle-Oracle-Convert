...
include("mydbPConnection.php");
	...
	$orgid=@$_GET['org_id'];
	//echo "$orgid";
	$rotation=mysqli_query($con_cchaportdb,"select Import_Rotation_No from igm_details where IGM_id='$from'");
		...
		<TR>
			<TD width="100%">
				<table class='table-header' border=0 width="100%">
					<tr>
						<td colspan="2" align="center"><h1> DISCHARGE LIST  </h1></td>
					</tr>
					<tr>						
							<?php
							$result_now=mysqli_query($con_cchaportdb,"SELECT NOW() as datetime");
							$row_now=mysqli_fetch_object($result_now);
			...
			$str1=mysqli_query($con_cchaportdb,"select ETA_Date,ETD_Date,Actual_Berth,Actual_Berth_time from vessels_berth_detail
									where Import_Rotation_No='$import'");
$row_time=mysqli_fetch_object($str1);
			$sl=1;
		?>
		<table>
			<tr>
...
$sad_reg_nber=substr($row1->Bill_of_Entry_No,1);
							$bin=mysqli_query($con_cchaportdb,"select  SAD_CONSIGNEE,SAD_DECLARANT from sad_gen_informations where SAD_REG_NBER='$sad_reg_nber' 
...
navyresponse.rejected_application,navyresponse.final_amendment from igm_navy_response navyresponse inner join  igm_details igms on igms.id=navyresponse.igm_details_id where igms.id = $row1->id";
							...
							$navy=mysqli_query($con_cchaportdb,$str5);
						$rownavy=mysqli_fetch_object($navy);	
					  ...
					  <td  align="center">
						...
						$sql_agent_code=mysqli_query($con_cchaportdb,"select mlo_agent_code_ctms from mlo_detail 
	...
	<td  align="center"><?php 
						...
						if ($num_rows>0)
					{
						...
						$resultnavyff=mysqli_query($con_cchaportdb,$str5);
						$rownavyff = @mysqli_fetch_object($resultnavyff);
						$result3 = mysqli_query($con_cchaportdb,"select cons.id, cons.igm_detail_id,cons.Consignee_ID from igm_detail_consigneetabs cons where cons.igm_detail_id=$row1->id");						
