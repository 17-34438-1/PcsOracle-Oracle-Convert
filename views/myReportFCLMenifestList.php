...
where igms.Import_Rotation_No='$rotation'  and igm_detail_container.cont_status='FCL' and igm_supplimentary_detail.id is null and igms.final_submit=1 and off_dock_id <> 2592
union
...
where igms.Import_Rotation_No='$rotation'  and igm_sup_detail_container.cont_status='FCL' and igms.final_submit=1 and off_dock_id <> 2592";


//print($str);
//Its for other organization................. close it for new port panel only
/*else
...
<table align="center">
			...
			<tr><td style="font-size:22px;" ><b>FCL MENIFEST</b></td></tr>
...
<?php
		...
		?>
			    ...
			    <tr class='gridLight'>
					...
					$strLoading=mysqli_query($con_cchaportdb,"select Description from Locationcode where Port_Code='$row->place_of_unloading'");
					$rowLoading=@mysqli_fetch_object($strLoading);
					...
					<table width="100%">
					<tr><td>Off-dock Name</td><td>Cnt.&nbsp;Number</td><td>Seal&nbsp;Number</td><td>Size</td><td>Type</td><td>Height</td><td>Gross Weight(Kgs)</td><td>Tare Weight(Kgs)</td><td>Status</td><td>IMCO</td><td>UN</td></tr>
					<?php 
					//load container detail
						...
						if ($row->type=="D")
						$str="select cnt.id as id, cnt.cont_number as cont_number, cnt.cont_size as cont_size,cnt.cont_type as cont_type,cnt.cont_height as cont_height,cnt.cont_status as cont_status,cnt.cont_gross_weight as gross_weight,cnt.cont_weight as cont_weight,cnt.cont_seal_number as cont_seal_number,cnt.cont_description as cont_description,cnt.cont_imo as cont_imo, cnt.cont_un as cont_un,off_dock_id,Organization_Name from igm_detail_container cnt 
inner join organization_profiles on organization_profiles.id=cnt.off_dock_id
where cnt.igm_detail_id=$row->id and cnt.cont_status='FCL'  and off_dock_id <> 2592";
					else
					$str="select cnt.id as id, cnt.cont_number as cont_number, cnt.cont_size as cont_size,cnt.cont_type as cont_type,cnt.cont_height as cont_height,cnt.cont_status as cont_status,cnt.cont_gross_weight as gross_weight,cnt.cont_weight as cont_weight,cnt.cont_seal_number as cont_seal_number,cnt.cont_description as cont_description,cnt.cont_imo as cont_imo, cnt.cont_un as cont_un,off_dock_id,Organization_Name from igm_sup_detail_container cnt 
inner join organization_profiles on organization_profiles.id=cnt.off_dock_id
where cnt.igm_sup_detail_id=$row->id and cnt.cont_status='FCL' and off_dock_id <> 2592";						
						
					$result1 = mysqli_query($con_cchaportdb,$str);		
						while($row1 = mysqli_fetch_object($result1)) {
							print("<tr><td>$row1->Organization_Name</td><td>$row1->cont_number</td><td>$row1->cont_seal_number</td><td>$row1->cont_size</td><td>$row1->cont_type</td><td>$row1->cont_height</td><td>$row1->gross_weight</td><td>$row1->cont_weight</td><td>$row1->cont_status</td><td>$row1->cont_imo</td><td>$row1->cont_un</td></tr>");	
