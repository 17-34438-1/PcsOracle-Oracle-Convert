...
<HTML>
	...
	<?php } else if($_POST['options']=='xl'){
		...
		header("Content-Disposition: attachment; filename=IGMCONT.xls;");
...
*/
	...
	$awstatus="";
		...
		$str="select 
							...
							from 
								igm_details igms inner join igm_detail_container on 								   igm_detail_container.igm_detail_id=igms.id left join igm_supplimentary_detail on igm_supplimentary_detail.igm_detail_id=igms.id 
							where 
								upper(cont_number) like upper('%$_POST[description]%') and igm_supplimentary_detail.igm_detail_id is null
...
select 
							...
							from 
								igm_supplimentary_detail igms inner join igm_sup_detail_container on 								   igm_sup_detail_container.igm_sup_detail_id=igms.id 
							where 
								upper(cont_number) like upper('%$_POST[description]%') 
