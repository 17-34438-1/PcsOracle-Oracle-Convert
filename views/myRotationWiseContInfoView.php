<?php
	...
	$sql="select igm_masters.Vessel_Name from igm_masters where Import_Rotation_No='$ddl_imp_rot_no'";
	$sql=mysqli_query($con_cchaportdb,$sql);
	...
	// if(file_exists($_SERVER['DOCUMENT_ROOT']."/portpanel/resources/Container/".$myFile))
	// {
			// unlink($_SERVER['DOCUMENT_ROOT']."/portpanel/resources/Container/".$myFile);
	// }
	
	if(file_exists($_SERVER['DOCUMENT_ROOT']."/resources/Container/".$myFile))
	{
			unlink($_SERVER['DOCUMENT_ROOT']."/pcs/resources/Container/".$myFile);
	}
	
	// $fh= fopen($_SERVER['DOCUMENT_ROOT']."/myportpanel/resources/Container/".$myFile , 'a');
	$fh= fopen($_SERVER['DOCUMENT_ROOT']."/pcs/resources/Container/".$myFile , 'a');
	 $query="select distinct Organization_Name,cont_number,cont_size,cont_gross_weight,
                                  ...
                                  igm_details.IGM_id=igm_masters.id left join organization_profiles on organization_profiles.id=
                                  igm_detail_container.org_id where igm_details.Import_Rotation_No='$ddl_imp_rot_no'";
								  // left join added
	/*							  
	// edited by awal
	$query=mysql_query("select distinct REPLACE(Organization_Name,'S','\'') AS Organization_Name, REPLACE(cont_number,'S','\'') AS cont_number,cont_size,cont_gross_weight,
                                  cont_weight,cont_seal_number,cont_status,off_dock_id,cont_imo,
                                  cont_un,commudity_code,cont_height,cont_iso_type,cont_type,mlocode,igm_masters.Port_Ship_ID,
                                  REPLACE(Vessel_Name,' ','_' ) AS Vessel_Name,
                                  (select Organization_Name from organization_profiles
                                  ...
                                  igm_detail_container.org_id where igm_details.Import_Rotation_No='$ddl_imp_rot_no'");
	// edited by awal*/
	$query=mysqli_query($con_cchaportdb,$query);
	while($row=mysqli_fetch_object($query)){
	echo $contDatastart	=''.$row->cont_number.'|'.$row->cont_size.'|'.$row->cont_gross_weight.'|'.$row->cont_weight.'|'.$row->cont_seal_number.'|'.$row->cont_status.'|'.$row->cont_height.'|'.$row->cont_iso_type.'|'.$row->cont_type.'|'.$row->offdock_name.'|'.$row->offdock_name.'|'.$row->Organization_Name.'|'.$row->mlocode.'|'.$row->cont_imo.'|'.$row->cont_un.'|'.$row->Port_Ship_ID.'|'.$row->commodity.'';
...
if (file_exists($_SERVER['DOCUMENT_ROOT']."/pcs/resources/Container/".$myFile))
 {
  $file = $_SERVER['DOCUMENT_ROOT']."/pcs/resources/Container/".$myFile;
  $fp = fopen($file, 'rb');
