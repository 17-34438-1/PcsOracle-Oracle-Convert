...
/********************************************Start of transfer data to CTMS Manifest***********************************************/
	...
	/*if (file_exists('/var/www/html/manifest/manifest/'.$myFile_old)) 
		...
		// break;
	if(file_exists($_SERVER['DOCUMENT_ROOT']."/pcs/resources/manifest/".$myFile_old))
	{
		unlink($_SERVER['DOCUMENT_ROOT']."/pcs/resources/manifest/".$myFile_old);
	}
...
//read_datetime is null
	...
	$igm_master_id_old="";
	  ...
	  $stringData = "</edi:blTransactions>\n";
			...
			if (file_exists($_SERVER['DOCUMENT_ROOT']."/pcs/resources/manifest/".$myFile)) {
				...
				header('Content-Length: ' . filesize($_SERVER['DOCUMENT_ROOT']."/pcs/resources/manifest/".$myFile));
				
				ob_end_clean(); 
				flush();
				readfile($_SERVER['DOCUMENT_ROOT']."/pcs/resources/manifest/".$myFile);
...
$ip=$this->session->userdata('ip_address');
	...
	$handle111= fopen($_SERVER['DOCUMENT_ROOT'].'/pcs/resources/manifest/CTMS'.$date.'log.txt' , 'a') or exit("Unable to open file!"); 
