	...
	<form class="form-horizontal form-bordered" method="POST" action="<?php echo base_url().'index.php/Report/cnfCertifyInsert'; ?>" target="" id="myform" name="myform" onsubmit="return validate()">
		...
		<thead>
			<tr class="gridDark" style="cursor: pointer;" title="Click to sort">
				...
				<th align="center" onclick="sort_table(data, 0, asc1); asc1 *= -1; asc2 = 1; asc3 = 1;">Assignment Date</th>
		...
		?>
			    ...
			    <tr <?php if(strtoupper($rtnContainerList[$i]['cont_number'])==strtoupper($containerNo)) { ?> class="pinkLight" <?php } else { ?> class="gridLight" <?php } ?>>
					...
					<?php
						...
						$AssigntypeQuery=mysqli_query($con_cchaportdb,"SELECT mfdch_value,assignmentDate FROM assignment_request_data WHERE cont_number='$cont' AND rotation='$rotation'");
					...
					<td align="center">
						<select class="form-control" id="assignType<?php echo $i; ?>" style="padding:2px;" name="assignType<?php echo $i; ?>">
							<?php
					...
					<td align="center">
						<input type="date" class="form-control" style="padding:3px;width:135px;" name="assignmentDate<?php echo $i; ?>" id="assignmentDate<?php echo $i; ?>" value="<?php if($assignTypeByCont != null){if($assignTypeByCont->assignmentDate !=null){ echo $assignTypeByCont->assignmentDate;}else{echo date('Y-m-d', strtotime(' +1 day'));}} ?>"/>
					</td>

					<td align="center" ><?php if($yard != "" && $block != ""){echo $yard.", ".$block;}  ?></td>
