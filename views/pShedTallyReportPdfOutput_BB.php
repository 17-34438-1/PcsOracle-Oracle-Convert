					...
					<table border="1" align="center" width="100%">
						...
						<?php 
							...
							while($rtntallyreport=mysqli_fetch_object($restallyreport)) {
								...
								$strrcv = "select shed_tally_info.id, sum(rcv_pack) as rcv_pack,sum(flt_pack) as flt_pack,sum(flt_pack_loc) as flt_pack_loc,sum(loc_first) as loc_first,sum(total_pack) as total_pack,rcv_unit,shed_yard,shed_loc,
								...
								UNION ALL 
								SELECT shed_tally_info.id, SUM(rcv_pack) AS rcv_pack,SUM(flt_pack) AS flt_pack,SUM(flt_pack_loc) AS flt_pack_loc,SUM(loc_first) AS loc_first,SUM(total_pack) AS total_pack,rcv_unit,shed_yard, shed_loc,remarks,
								imdg,sum(weight) as weight  FROM shed_tally_info 
								WHERE igm_detail_id='$rtntallyreport->id'  AND update_by='pshed'  GROUP BY rcv_unit,shed_loc 
								ORDER BY id";
								...
								from shed_tally_info 
								where igm_sup_detail_id='$rtntallyreport->id' OR igm_detail_id='$rtntallyreport->id'  AND update_by='pshed'
								group by rcv_unit,shed_loc
						...
						<tr>
								...
								<?php
									$marksVal = "";
									 $strMarks = "SELECT DISTINCT(actual_marks) AS actual_marks FROM shed_tally_info 
											WHERE igm_detail_id='$rtntallyreport->id' AND  shed_tally_info.update_by='pshed'
											UNION 
											select distinct(actual_marks) as actual_marks from shed_tally_info 
											where igm_sup_detail_id='$rtntallyreport->id' AND  shed_tally_info.update_by='pshed'";
							...
							<?php
								...
								where igm_sup_detail_id='$rtntallyreport->id' 
								UNION 
								SELECT COUNT(DISTINCT(rcv_unit)) AS totRcvUnit FROM shed_tally_info 
								where igm_detail_id='$rtntallyreport->id'";
								...
								$strUnits = "
										SELECT DISTINCT(rcv_unit) AS rcvUnit FROM shed_tally_info 
										where igm_detail_id='$rtntallyreport->id'";
								$resUnits = mysqli_query($con_cchaportdb,$strUnits);
								while($rowUnits = mysqli_fetch_object($resUnits)){
									$forIgmSupDtl++;
									$strTotWght = "SELECT * FROM (SELECT SUM(total_pack) AS totPack,SUM(weight) AS weight,shed_yard,shed_loc,wr_date
									FROM shed_tally_info WHERE rcv_unit='$rowUnits->rcvUnit' AND igm_sup_detail_id='$rtntallyreport->id'
									UNION
									SELECT SUM(total_pack) AS totPack,SUM(weight) AS weight,shed_yard,shed_loc,wr_date
									FROM shed_tally_info WHERE rcv_unit='$rowUnits->rcvUnit' AND igm_detail_id='$rtntallyreport->id') AS tmp WHERE  totPack IS NOT NULL";
									$resTotWght = mysqli_query($con_cchaportdb,$strTotWght);
									$rowTotWght = mysqli_fetch_object($resTotWght);
									
									$subTotQty = $rowTotWght->totPack;
									$totQty = $totQty+$subTotQty;
							?>
							<td align="center" style="font-size:9px;"><?php echo $rowTotWght->totPack."<br>"; ?></td>
							<td align="center" style="font-size:9px;"><?php echo $rowUnits->rcvUnit."<br>";?></td>
							<td align="center" style="font-size:9px;"><?php echo $rowTotWght->weight."<br>";?></td>
							<td align="center" style="font-size:9px;"><?php echo $rowTotWght->shed_loc."<br>";?></td>
								</tr>
							<tr>
							<?php }  
							$strTotalUnits = "SELECT COUNT(DISTINCT(rcv_unit)) AS totalUnit FROM shed_tally_info 
									where igm_sup_detail_id='$rtntallyreport->id'
									UNION
									SELECT COUNT(DISTINCT(rcv_unit)) AS totalUnit FROM shed_tally_info 
									where igm_detail_id='$rtntallyreport->id'";
