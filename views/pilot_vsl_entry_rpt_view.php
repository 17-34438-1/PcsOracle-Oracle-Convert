<html>
	...
	<body>
		...
		<?php
			$strPilotVesselEntry= "SELECT id,vsl_name,master_name,flag,grt,nrt,deck_cargo,loa,local_agent,last_port,next_port,rotation, entry_by FROM doc_vsl_info where status=0";
			//echo $strAssignment;
			$queryPilotVsl=mysqli_query($con_cchaportdb,$strPilotVesselEntry);
			$i=0;
			while($rowPilotVessel=mysqli_fetch_object($queryPilotVsl))
			{
			$i++;			
		?>
			<tr align="center">
					...
					<td class="contact-delete">
						<?php if(strtoupper($this->session->userdata('login_id'))==strtoupper($rowPilotVessel->entry_by) OR strtoupper($this->session->userdata('login_id'))=='ADMIN'){?>
