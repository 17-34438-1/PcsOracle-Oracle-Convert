...
<BODY>
 ...
 <?php } 
  ...
  header("Content-Disposition: attachment; filename=Rotation_Wise_PreAdvise_Container_Report.xls;");
...
$strVsl = "select name from vsl_vessel_visit_details
inner join vsl_vessels on vsl_vessels.gkey=vsl_vessel_visit_details.vessel_gkey
where ib_vyg='$rotation'";

$rslt_n4ISOType = oci_parse($con_sparcsn4_oracle, $strVsl);
oci_execute($rslt_n4ISOType);

$name="";
while(($row1=oci_fetch_object($rslt_n4ISOType)) !=false)
		{
			$name=$row1->NAME;
			
		}
	
// $resVsl = mysqli_query($con_sparcsn4,$strVsl);
// $rowVsl=mysqli_fetch_object($resVsl)
?>
...
<tr bgcolor="#ffffff" align="center" height="50px">
		<td colspan="14" align="center" style="border:none;">
			...
			<font size="5"><b><?php echo "Rotation: ".$rotation." Vessel Name: ".$name;?></b></font>
		</td>		
	</tr>
...
$res = mysqli_query($con_sparcsn4,$strQuery);
//echo $strQuery;
$i=0;
while($row=mysqli_fetch_object($res)){
	$i++;
	//echo $i;
	$strTrans = "select inv_unit_fcy_visit.transit_state,inv_unit.category,inv_unit_fcy_visit.last_pos_name from inv_unit 
	...
	$cat="";
	$lastPos = "";
	while(($row1=oci_fetch_object($resTrans)) !=false)
			{
	...
	// while($rowTrans = oci_fetch_object($resTrans))
	// {
	// 	$Trans=$rowTrans->transit_state;
	// 	$cat=$rowTrans->category;
	// 	$lastPos=$rowTrans->last_pos_name;
