		...
		<table width="100%" cellpadding="0" border="0">
			<tr height="100px">
				<td colspan="5" align="center" valign="middle">
					...
					<h3>RELEASE ORDER FOR DELEVERY (CASH) <?php if($rtnContainerList[$i]['pr_number']){ ?> P.R. NO <u><?php echo $rtnContainerList[$i]['pr_number']; ?></u><?php } ?> C.P NO <u>
					<?php 
			...
			<tr>
				<td align="center" >
					EX.S.S <u><?php if(isset($rtnContainerList[$i]['vessel_Name'])) {echo $rtnContainerList[$i]['vessel_Name'];} else echo ""; ?></u>
				</td>
				<td align="center" >
					ROTATION <u><?php if(isset($rtnContainerList[$i]['Import_Rotation_No'])) echo  $rtnContainerList[$i]['Import_Rotation_No']?></u>
				</td>
				<td align="center">
					LINE/BL <u><?php if(isset($rtnContainerList[$i]['BL_No'])) echo $rtnContainerList[$i]['BL_No']?></u>
				</td>
				<td align="center">
				...
				<td rowspan="2">
					<?php 
						...
						for($j=0;$j<count($rtnContainerList);$j++)
						{
							...
							WHERE inv_unit.id='".$rtnContainerList[$j]['cont_number']."' AND ib_vyg='".$rtnContainerList[$i]['Import_Rotation_No']."'";
							...
							{
								$pos = $row->POS;
								$queryYard="SELECT ctmsmis.cont_yard('$pos') AS Yard_No";
								$strYard = mysqli_query($con_sparcsn4,$queryYard);
								while($rowYard= mysqli_fetch_object($strYard)){
									$yard_No = $rowYard->Yard_No;
								}	
							}
				...
				<td valign="top">
					<table border="0">
						...
						<tr height="30px" valign="bottom">
							<td colspan="2"><b>CFS/Shed/Yard: <u><?php if($rtnContainerList[$i]['cont_status']=="LCL"){ echo  $rtnContainerList[$i]['shed_yard']; }else{ echo $yard_No; }?></u></b></td>
						</tr>
						<tr height="30px" valign="bottom">
							<td colspan="2"><b>Location: <u><?php if($rtnContainerList[$i]['cont_status']=="LCL"){ echo  $rtnContainerList[$i]['shed_loc']; }else{ echo $pos; } ?></u><b></td>
						</tr>
						<tr>
							<td>S.S. -------------</td>
							<td>Date -------------</td>
						...
						<tr>
							<td>Reg. No. <u><?php echo  $rtnContainerList[$i]['Import_Rotation_No']?></u></td>
