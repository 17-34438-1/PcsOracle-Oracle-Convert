		...
		<table width="100%" cellpadding="0" border="0" class="tblFont">
			<tr>
				...
				<td colspan="3" style="padding-left:80px;" align="center" width="70%">
					...
					<h4>RELEASE ORDER FOR DELEVERY (CASH) C.P NO <u><?php echo $cpNo; ?></u> OF <u><?php echo $cpDate; ?></u></h4>
				</td>
					...
					<table border="0" style="border-collapse:collapse;" width="100%">
						...
						<?php
							...
							else
							{
								...
								INNER JOIN used_equipment ON used_equipment.equipment_id = appraisement_info.equipment_id
		...
		<table border="1" width="100%" style="border-collapse:collapse">
			...
			for($k=0;$k<count($rslt_roData);$k++)
			{
				...
				if($contStatus=="FCL")
				{
					...
					inv_unit_fcy_visit.last_pos_slot AS YARD_NO
					FROM inv_unit
					...
					$strPosForFcl = $this->bm->dataSelect($strQuerypos);
				}
				...
				else if($contStatus=="LCL")
				{
					...
					$strPosForFcl = $this->bm->dataInsertDB1($strQuerypos);
				}

				$pos = null;
				$yard_No = null;
				if(count($strPosForFcl)>0){
					$pos = $strPosForFcl[0]["POS"];
					$yard_No = $strPosForFcl[0]["YARD_NO"];
				}
				// location - end

				// CL Date - start

				$cl_query = "SELECT TO_CHAR(time_discharge_complete,'DD-MM-YYYY hh24:mi:ss') AS CL_DATE
				FROM argo_visit_details
				INNER JOIN vsl_vessel_visit_details on vsl_vessel_visit_details.vvd_gkey = argo_visit_details.gkey
				WHERE vsl_vessel_visit_details.ib_vyg = '$rotMain'";
				$cl_data = $this->bm->dataSelect($cl_query);
				
				$cl_date = null;
				if(count($cl_data)>0)
				{
					$cl_date = $cl_data[0]["CL_DATE"];
				}
				
				// CL Date - end

				// // MLO and FF Status - start

				$rot_no = $rotMain;
				$bl_no = $rslt_headerData[0]['BL_No'];
				...
				if($type_of_igm!="")
				{
					...
					{	
						...
						for($j=0;$j<count($resltStr);$j++)
						{
							$Submitee_Org_Id=$resltStr[$j]['Submitee_Org_Id'];
						}
						if($Submitee_Org_Id!="")
							{
								...
								for($j=0;$j<count($resSHDtls);$j++)
								{
									$shippingAgentName=$resSHDtls[$j]['Organization_Name']." - (".$resSHDtls[$j]['ain'].")";
								}
							}
					}
					else
					{
						...
						if(count($rsltStr)>0)
						{
							...
							for($j=0;$i<count($rsltStr);$j++)
							{
								$sup_org=$rsltStr[$j]['sup_org'];
								$master_org=$rsltStr[$j]['master_org'];
								$mloId=$rsltStr[$j]['Submitee_Id'];
							}
							...
							if($sup_org!="")
							{
								...
								for($j=0;$j<count($rowFFDtls);$j++)
								{
									$ffName=$rowFFDtls[$j]['Organization_Name']." - (".$rowFFDtls[$j]['ain'].")";
								}
							...
							if($master_org!="")
							{
								...
								for($j=0;$j<count($resMloDtls);$j++)
								{
									$mloName= $resMloDtls[$j]['Organization_Name']." - (".$resMloDtls[$j]['ain'].")";
								}
			...
			<tr>
				...
				<td align="center"><?php echo $mloName; ?></td>
				<td align="center"><?php echo $ffName; ?></td>
				...
				<tr>
					<td width="40%" style="vertical-align:text-top">
						<table width="100%" border="1" style="border-collapse:collapse">
							...
							<tr>
								<td>CP No. : <b><?php echo $cpNo; ?></b></td>
								<td>Date : <b><?php echo $cpDate; ?></b></td>
							</tr>
							<!--tr>
								<td colspan="2">&nbsp;</td>
							</tr-->
