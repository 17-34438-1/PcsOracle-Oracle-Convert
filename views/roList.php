					...
					<table class="table table-bordered table-hover table-striped mb-none" id="datatable-default">
						...
						<?php 
							...
							for($i=0;$i<$count;$i++)
							{
								...
								$chkverify = "SELECT COUNT(verify_number) AS rtnValue,verify_time,verify_number
								FROM igm_details 
								LEFT JOIN verify_info_fcl ON verify_info_fcl.igm_detail_id=igm_details.id
								WHERE igm_details.Import_Rotation_No = '$rotation' AND igm_details.BL_No = '$bl' AND verify_number IS NOT NULL";
								...
								if(count($chkverifiedRslt)>0)
								{
									...
									$verify_no = $chkverifiedRslt[0]['verify_number'];
								}
								...
								if($chkverified == 0){
									...
									WHERE igm_supplimentary_detail.Import_Rotation_No = '$rotation' AND igm_supplimentary_detail.BL_No = '$bl' AND verify_number IS NOT NULL";
									$chkverifiedRslt = $this->bm->dataSelectDb1($chkverify);
									if(count($chkverifiedRslt)>0)
									{
										$chkverified = $chkverifiedRslt[0]['rtnValue'];
