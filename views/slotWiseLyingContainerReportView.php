    ...
    <div align="center">
	   ...
	   include_once("FrontEnd/mydbPConnection.php");
		...
		if($cont_type=='Reefer')
		{
			...
			{
				$sql_cond="AND sparcsn4.ref_equip_type.iso_group  IN ('RE','RS','RT')
						AND DATE(vsl_vessel_visit_details.flex_date08) <= (DATE_ADD(DATE(NOW()), INTERVAL -366 DAY))";
			}
		}
		else if($cont_type=="Normal" or $cont_type=="IMDG")
		{
			...
			else if($day_slot=="1y+")
			{
				$sql_cond="AND sparcsn4.ref_equip_type.iso_group  NOT IN ('RE','RS','RT')
				 ...
				 $sql = "SELECT inv_unit.id as cont_no,vsl_vessel_visit_details.ib_vyg,
			...
			date(sparcsn4.vsl_vessel_visit_details.flex_date08) as flex_date08, sparcsn4.ref_equip_type.iso_group,
			...
			$i++;
					...
					//$rowcount=mysqli_num_rows($res_igm);
			...
			<table width="30%" border ='1' cellpadding='0' cellspacing='0'>
				...
				<tr align="center" bgcolor="#FCF7F7">
					...
					<td><b><?php echo  $forty; ?></b></td>
					<td><b><?php echo  $twenty; ?></b></td>
				</tr>	
			...
			$i++;
				...
				<tr align="center" bgcolor="#FCF7F7">
					...
					<td align="left" style="padding-left: 10px;"><?php if($row_igm->Notify_name) echo($row_igm->Notify_name); else echo("&nbsp;");?></td>
