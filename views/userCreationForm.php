		...
		xmlhttp.onreadystatechange=function()
		{
			...
			{							  
				...
				if(jsonData.result.length>0)
				{
					...
					{
						id = jsonData.result[i].id;
						short_name = jsonData.result[i].short_name;
						select.options[select.options.length] = new Option(short_name,id);
					}
				}
			}
		};
		
		var url = "<?php echo site_url('AjaxController/getSectionDetail')?>?orgType="+orgType;
							...
							<div class="col-md-6">		
								...
								<div class="input-group mb-md">
									...
									<input class="form-control login_input_text" id="license_no" name="license_no" type="text" 
										value="<?php if(isset($rslt_user_data[0]['License_No'])) echo $rslt_user_data[0]['License_No'];?>"
								...
								<div class="input-group mb-md">
									...
									$org_id="";
									$u_type="";
									if(isset($rslt_user_data[0]['u_type']))
									{
										$u_type=$rslt_user_data[0]['u_type'];
										$org_id=$rslt_user_data[0]['org_id'];
										if($u_type=='single')
										{
											$sql_org_name="SELECT id,Organization_Name FROM organization_profiles WHERE organization_profiles.id='$org_id'";		
											$rslt_org_name=mysqli_query($con_cchaportdb,$sql_org_name);									
											while($row_org_name=mysqli_fetch_object($rslt_org_name))
											{
													 $org_name=$row_org_name->Organization_Name; 
											} 
										} 
									} ?>
									<input class="form-control login_input_text" list="org_names" name="orgName" id="orgName" value="<?php if(isset($rslt_user_data[0]['u_name'])){ if($u_type=='single') { echo $org_name; }   else { echo $rslt_user_data[0]['u_name']; } } ?>" autocomplete="off" required />
									<datalist id="org_names" autocomplete="off">
									<?php
									//$sql_org_name="SELECT id,Organization_Name FROM organization_profiles";
									$sql_org_name="SELECT organization_profiles.id,organization_profiles.Organization_Name,tbl_org_types.Org_Type
									...
									{
									?>
										<option value="<?php echo $row_org_name->Organization_Name.":-".$row_org_name->Org_Type;?>" id="<?php echo $row_org_name->id; ?>" label="">
										</option>
									<?php
									}
									?>											
								...
								<div id="div_uname"  style="display:none;">
									...
									value="<?php if(isset($rslt_user_data[0]['u_name'])) echo $rslt_user_data[0]['u_name'];?>" 
									<?php if(isset($rslt_user_data[0]['u_name'])) ;?>
										>
									</div>
								...
								<div class="input-group mb-md">
									...
									<input type="text" class="form-control" name="cPhone" id="cPhone" 
										value="<?php if(isset($rslt_user_data[0]['Cell_No_1'])) echo $rslt_user_data[0]['Cell_No_1'];?>" required>
								</div>
								<div class="input-group mb-md">
								...
								<div class="input-group mb-md">
									...
									<?php
										if(isset($rslt_user_data[0]['section'])){
											$sectionIdEdit = $rslt_user_data[0]['section'];
											$sectionNameQueryEdit = "SELECT short_name FROM users_section_detail WHERE id = '$sectionIdEdit'";
											$sectionNameResultEdit = $this->bm->dataSelectDB1($sectionNameQueryEdit);
											$sectionNameEdit = "";
											for($sn=0;$sn<count($sectionNameResultEdit);$sn++){
												$sectionNameEdit = $sectionNameResultEdit[$sn]['short_name'];
											}
										}
									?>
									<span class="input-group-addon span_width">Section</span>
									<select name="section" id="section" class="form-control">
										<option value=<?php if(isset($rslt_user_data[0]['section'])){ echo $rslt_user_data[0]['section'];}else{ echo "";}?>><?php if(isset($rslt_user_data[0]['section'])){ echo "$sectionNameEdit";}else{ echo "--Select--";}?></option>
										<!-- <option value="">--Select--</option> -->
									<?php
