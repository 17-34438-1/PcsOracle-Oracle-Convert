...
//for($i=0;$i<count($rtnTruckNumber);$i++)
	 ...
	 $cartDtl_str="SELECT do_truck_details_entry.id AS trucVisitId,verify_other_data_id,import_rotation,cont_no, delv_pack,actual_delv_pack, igm_pack_unit.Pack_Unit AS actual_delv_unit, agent_name,agent_code, traffic_chk_st,traffic_chk_by, traffic_chk_time,yard_security_chk_st,yard_security_chk_by,yard_security_chk_time,cnf_chk_st,cnf_chk_by,cnf_chk_time, truck_id,gate_no,driver_name,driver_gate_pass,assistant_name,assistant_gate_pass, igm_details.BL_No,igm_masters.Vessel_Name, oracle_nts_data.be_no, oracle_nts_data.be_dt, oracle_nts_data.verify_no,oracle_nts_data.cp_no,oracle_nts_data.cp_date,oracle_nts_data.unit_number,oracle_nts_data.office_code,cnf_lic_no, SUBSTR(igm_details.Description_of_Goods,1,100) AS Description_of_Goods,SUBSTR(igm_details.Pack_Marks_Number,1,100) AS Pack_Marks_Number,Consignee_name, agent_name AS jetty_sirkar_name,card_number AS jetty_sirkar_gate_pass,agent_photo AS jetty_sirkar_photo,igm_details.Notify_name
	...
	SELECT do_truck_details_entry.id AS trucVisitId,verify_other_data_id,import_rotation,cont_no, delv_pack,actual_delv_pack, igm_pack_unit.Pack_Unit AS actual_delv_unit, agent_name,agent_code, traffic_chk_st,traffic_chk_by, traffic_chk_time,yard_security_chk_st,yard_security_chk_by,yard_security_chk_time,cnf_chk_st,cnf_chk_by,cnf_chk_time, truck_id,gate_no,driver_name,driver_gate_pass,assistant_name,assistant_gate_pass, igm_supplimentary_detail.BL_No,igm_masters.Vessel_Name, oracle_nts_data.be_no, oracle_nts_data.be_dt, oracle_nts_data.verify_no,oracle_nts_data.cp_no,oracle_nts_data.cp_date,oracle_nts_data.unit_number,oracle_nts_data.office_code,cnf_lic_no, SUBSTR(igm_supplimentary_detail.Description_of_Goods,1,100) AS Description_of_Goods,SUBSTR(igm_supplimentary_detail.Pack_Marks_Number,1,100) AS Pack_Marks_Number,Consignee_name, agent_name AS jetty_sirkar_name,card_number AS jetty_sirkar_gate_pass,agent_photo AS jetty_sirkar_photo,igm_supplimentary_detail.Notify_name
	FROM do_truck_details_entry 
	INNER JOIN lcl_dlv_assignment ON lcl_dlv_assignment.id = do_truck_details_entry.verify_other_data_id
	INNER JOIN vcms_vehicle_agent ON lcl_dlv_assignment.jetty_sirkar_id = vcms_vehicle_agent.id
...
/* 	
	...
	agent_name AS jetty_sirkar_name,card_number AS jetty_sirkar_gate_pass,agent_photo AS jetty_sirkar_photo
	FROM do_truck_details_entry
	LEFT JOIN lcl_dlv_assignment ON lcl_dlv_assignment.id=do_truck_details_entry.verify_other_data_id
	LEFT JOIN vcms_vehicle_agent ON lcl_dlv_assignment.jetty_sirkar_id = vcms_vehicle_agent.id
	...
	$YardQuery = "SELECT shed_yard FROM shed_tally_info WHERE cont_number='$cont_no'  AND import_rotation='$rot_no' LIMIT 1 ";
...
<body>
	...
	<?php
		if($rtnCartTicket[0]['actual_delv_pack']>0) { 
			...
			$destination_folder = $_SERVER['DOCUMENT_ROOT']."/pcsTest/assets/images/qrcode/";
	...
	<table border="0" width="100%" style="padding-top:1px;margin-top:1px;">
		...
		<tr>
			...
			<td colspan="3" style="border-bottom:1px solid black;">
				<?php
					// echo preg_replace('/[^A-Za-z0-9,;()@&\-]/', ' ', $rtnCartTicket[0]['Consignee_name']);
					echo preg_replace('/[^A-Za-z0-9,;()@&\-]/', ' ', $rtnCartTicket[0]['Notify_name']);
				?>
			</td>
		</tr>
		<tr>
	...
	<table border="1" width="100%" align="center" style="padding-bottom:1px;margin-bottom:1px;">
		...
		<tr align="center">			
			...
			<td rowspan="<?php echo $cntHBL+1; ?>" align="center" style="font-size:11px;">
				<font size="5">
					<?php 
						echo $rtnCartTicket[0]['actual_delv_pack'];
					?>
				</font>
			</td>
			<td rowspan="<?php echo $cntHBL+1; ?>" align="center" style="font-size:11px;">
		...
		<tr align="center">			
			<td width="30%" style="font-size:11px;text-align:center;">
				<?php 
					echo preg_replace('/[^A-Za-z0-9,;()@&\-]/', ' ', $rtnCartTicket[0]['Pack_Marks_Number']);
				?>
			</td>
			<!-- <td width="30%" style="font-size:11px;text-align:center;"><?php echo $resHBL[$k]['BL_No']; ?></td> -->
			<td width="30%" style="font-size:11px;text-align:center;">
				<?php 
					echo preg_replace('/[^A-Za-z0-9,;()@&\-]/', ' ', $resHBL[$k]['Description_of_Goods']);
				?>
			</td>
		</tr>
		<?php } ?>
		...
		<?php  } else { ?>
		<tr align="center">
			<td rowspan="2" width="30%" style="font-size:10px;text-align:center;">
				<?php
					echo preg_replace('/[^A-Za-z0-9,;()@&\-]/', ' ', $rtnCartTicket[0]['Pack_Marks_Number']);
				?>
			</td>
			...
			<td rowspan="2" width="30%" style="font-size:10px;text-align:center">
				<?php
					echo preg_replace('/[^A-Za-z0-9,;()@&\-]/', ' ', $rtnCartTicket[0]['Description_of_Goods']);
				?>
			</td>
	...
	<?php if(count($rsltExtraTrucks) > 0) { ?>
		...
		<?php for($i=0;$i<count($rsltExtraTrucks);$i++){ ?>
			...
			<table border="1" width="100%" align="center" style="padding-bottom:1px;margin-bottom:1px;">
				...
				<tr align="center">
					<td rowspan="2" width="30%" style="font-size:10px;">
						<?php
							echo preg_replace('/[^A-Za-z0-9,;()@&\-]/', ' ', $rtnCartTicket[0]['Pack_Marks_Number']);
						?>
					</td>
					<td rowspan="2" width="40%" style="font-size:10px;">
						<?php 
							echo preg_replace('/[^A-Za-z0-9,;()@&\-]/', ' ', $rtnCartTicket[0]['Description_of_Goods']);
						?>
					</td>
...
<hr/>
    ...
    <table align="center" width="80%" border="1" style="font-size:12px;  border-collapse: collapse;">
        ...
        <tr>
            ...
            <td>
				<?php 
					echo preg_replace('/[^A-Za-z0-9,;()@&\-]/', ' ', $resQuery[0]['Notify_name']);
				?>
			</td>
        </tr>
        <tr>
