...
<section role="main" class="content-body">
	...
	<div class="col-lg-12">						
		...
		<div class="form-group">
			...
			<form method="POST" enctype="multipart/form-data" action="<?php echo site_url("Vessel/vesselForwardingForAccBillAction_Beached") ?>" <?php if($section=='acc') { ?> onsubmit="return validate()" <?php }?> >
	...
	<?php if($section=='acc') { ?>
			<table width=100%>
				...
				<tr>
					<td align="left">
						উপরোক্ত বিষয়ে জানানো যাচ্ছে যে , অত্র বিভাগের বার্থিং শাখায় জমাকৃত বন্দর বহিঃনোঙ্গর হইতে বিচ হয়েছে এ সংক্রান্ত জাহাজের বিবরণ সম্বলিত এক খানা তালিকা অর্থ ও হিসাব বিভাগে পরবর্তী প্রয়োজনীয় ব্যবস্থা গ্রহণের জন্য এতদসঙ্গে সংযুক্ত করে প্রেরণ করা হল। 
					</td>
				</tr>
			</table>
			<?php //} ?>		
				...
				<table class="table table-bordered table-hover table-striped mb-none" id="datatable-default">
						...
						<?php 
							for($i=0;$i<count($departData);$i++)
							{
								...
								$sql_offPortArr = "SELECT  to_char(off_port_arr,'yyyy-mm-dd') as off_port_arr
								...
								oci_execute($rslt_offPortArr);
								$offPortArr = "";
								while(($row_offPortArr = oci_fetch_object($rslt_offPortArr)) != false && !is_null($row_offPortArr->OFF_PORT_ARR))
								{
									$offPortArr = $row_offPortArr->OFF_PORT_ARR;
								}

								//echo $offPortArr;return;
							...
							<td>
								...
								?>
									...
									// if($departData[$i]['oa_date_dollar']!="" and $departData[$i]['oa_date_dollar']!=null and $isExist>0 and $isExistBillGenerate > 0 )
										if($departData[$i]['oa_date_dollar']!="" and $departData[$i]['oa_date_dollar']!=null) //and $isExist>0
										{
											if($departData[$i]['vsl_category'] == "BEACHING")
											{
									?>
									
												<a class="btn btn-primary" href="<?php echo site_url('VesselBill/Generate_Beaching_Bill/'.str_replace("/","_",$departData[$i]['ib_vyg']))?>" style="color:white" <?php if($chkBill>0){ ?>disabled<?php } ?> onclick="return confirMsg();">
													<u><nobr>Generate Bill</nobr></u>
												</a>
					...
					<table width=100%>				
						<tr>
							<td align="left">
								...
								<u>সংযুক্ত বর্ণনামতেঃ পাতা (০১) খানা তালিকা</u>
						</tr>
						<tr><td> <br/></td></tr>
						<!--tr>
							<td align="right">
