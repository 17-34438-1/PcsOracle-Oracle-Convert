...
<section role="main" class="content-body">
	...
	<div class="col-lg-12">						
		...
		<div class="form-group">
			...
			<form method="POST" enctype="multipart/form-data" action="<?php echo site_url("Vessel/vesselForwardingForAccBillAction_Cancelation") ?>" <?php if($section=='acc') { ?> onsubmit="return validate()" <?php }?> >
				...
				<table class="table table-bordered table-hover table-striped mb-none" id="datatable-default">
						...
						<?php 
							...
							if(count($depart_data)>0){
								...
								WHERE to_char(effective_date,'yyyy-mm-dd')='$oa_date_dollar'";
					...
					?>		
						<?php 
							...
							{ 
								...
								$rslt_chkBill = $this->bm->dataSelectDb2($sql_chkBill);
								
								$chkBill = 0;

								if(count($rslt_chkBill)>0)
								{
									$chkBill = $rslt_chkBill[0]['cnt'];
								}
							?>
							<td>
								<?php
								...
								else
								{
									...
									{									
								?>
										<a class="btn btn-primary"  href="<?php echo site_url('VesselBill/Generate_TugCancellation_Bill/'.str_replace("/","_",$departData[$i]['ib_vyg']))?>" style="color:white" <?php if($chkBill>0){ ?>disabled<?php } ?> target="_blank">
											Generate Bill
										</a>	
								<?php
									}
